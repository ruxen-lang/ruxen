//! Integration tests for `ruxen explain ECODE` long-form markdown
//! explanations (T5.04 phase 3).
//!
//! Phase 2 only printed the title from the central registry; phase 3
//! extends `ruxen explain` to print embedded markdown content (Why /
...
/// `ruxen-core` into the dev-dependency closure for tests that don't
/// otherwise need it (the CLI already depends on ruxen-core, but
/// integration tests run as a separate binary).
fn registered_codes() -> Vec<String> {
    let path = workspace_root().join("compiler/ruxen_core/src/diagnostics/codes.rs");
...
fn run_explain(code: &str) -> (bool, String, String) {
    let output = Command::new(ruxen_exe())
        .args(["explain", code])
        .output()
        .expect("invoke ruxen explain");
    (
...
fn explain_e0001_prints_long_form_sections() {
    ...
    assert!(ok, "ruxen explain E0001 failed; stderr: {stderr}");
    assert!(
