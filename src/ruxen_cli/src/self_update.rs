//! `ruxen update --from-source <path>` — rebuild + reinstall the
//! toolchain from a local ruxen checkout.
//!
...
pub fn from_source(path: &str) -> Result<(), String> {
    ...
    let abs_src = std::fs::canonicalize(path).map_err(|e| {
        format!(
            "`ruxen update --from-source {}`: path not found ({})",
            path, e
        )
    })?;
    ...
    if !abs_src.join("Cargo.toml").is_file() {
        return Err(format!(
            "`ruxen update --from-source {}`: not a ruxen checkout — no Cargo.toml at {}",
            path,
            abs_src.display()
        ));
    }
    if !abs_src.join("compiler/ruxen_core").is_dir() {
        return Err(format!(
            "`ruxen update --from-source {}`: doesn't look like ruxen — missing compiler/ruxen_core/ at {}",
            path,
            abs_src.display()
        ));
    }
    ...
    if !install_sh.is_file() {
        return Err(format!(
            "`ruxen update --from-source {}`: install.sh not found at {}",
            path,
            install_sh.display()
        ));
    }
