...
//!
//! Walks `src/` recursively and maps `.rx` files to module paths.
//! `src/http/client.rx` → `Http.Client`
...
pub struct ModuleNode {
    ...
    pub name: String,
    /// The `.rx` file for this module, if any.
...
impl ModuleTree {
    ...
    /// Get all files that need compilation (excluding main.rx and lib.rx entry points).
...
mod tests {
    ...
    fn create_test_project(dir: &Path) {
        ...
        fs::write(src.join("main.rx"), "def main\nend\n").unwrap();
        fs::write(src.join("utils.rx"), "pub def helper\nend\n").unwrap();
        fs::write(src.join("http.rx"), "# http module root\n").unwrap();
        fs::write(src.join("http/client.rx"), "pub class Client\nend\n").unwrap();
        fs::write(src.join("http/server.rx"), "pub class Server\nend\n").unwrap();
    }

    #[test]
    fn test_discover_modules() {
        let tmp = std::env::temp_dir().join(format!("ruxen_mod_test_{}", std::process::id()));
        ...
        // Root should have the main.rx file
    ...
    fn test_discover_nested_modules() {
        let tmp = std::env::temp_dir().join(format!("ruxen_nested_{}", std::process::id()));
        let _ = fs::remove_dir_all(&tmp);
        let src = tmp.join("src");
        fs::create_dir_all(src.join("http/client")).unwrap();

        fs::write(src.join("main.rx"), "").unwrap();
        fs::write(src.join("http/client/pool.rx"), "").unwrap();
    ...
    fn test_discover_empty_src() {
        let tmp = std::env::temp_dir().join(format!("ruxen_empty_{}", std::process::id()));
        let _ = fs::remove_dir_all(&tmp);
    ...
    fn test_discover_no_src() {
        let tmp = std::env::temp_dir().join(format!("ruxen_nosrc_{}", std::process::id()));
        let _ = fs::remove_dir_all(&tmp);
        fs::create_dir_all(&tmp).unwrap();
