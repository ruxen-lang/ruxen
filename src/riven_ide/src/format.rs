...
//! — thin adapter around `riven_core::formatter::format`.
//!
...
//!   support partial input (`format_range` in `riven_core` simply
...
pub fn format_range(source: &str, range: Range) -> Option<Vec<TextEdit>> {
    ...
    // `source[range]` and feed it through the formatter — Riven's
