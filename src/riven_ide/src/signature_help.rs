...
fn enclosing_call_context(source: &str, cursor: usize) -> Option<CallContext> {
    ...
    while i > 0 {
        ...
        match b {
            ...
            // Newlines are legal inside arg lists in Riven, so we do
            // NOT bail here. Riven uses no `;` statement separator.
            b'\n' => {}
            _ => {}
        }
    }
...
fn class_name_of(ty: &riven_core::hir::types::Ty) -> Option<String> {
...
enum Callee {
    ...
    Method {
        receiver_ty: riven_core::hir::types::Ty,
        method_name: String,
    },
}
        ...
        match item {
            ...
            HirItem::Class(class) => {
                ...
                for imp in &class.impl_blocks {
                    for it in &imp.items {
                        if let riven_core::hir::nodes::HirImplItem::Method(func) = it {
    ...
    fn visit_expr(&mut self, expr: &HirExpr) {
        ...
        match &expr.kind {
            HirExprKind::FnCall {
                ...
                // last write wins.)
                if *callee != riven_core::hir::nodes::UNRESOLVED_DEF {
...
fn format_signature(name: &str, sig: &FnSignature) -> String {
    let params: Vec<String> = sig
        ...
        .collect();
    let ret = if matches!(sig.return_ty, riven_core::hir::types::Ty::Unit) {
        String::new()
    } else {
