...
//!
//! Types are encoded using the `Ty: Display` implementation from `ruxen-core`,
...
fn generic_param_names(params: &[ruxen_core::hir::nodes::HirGenericParam]) -> Vec<String> {
    params
        .iter()
        .map(|g| {
...
fn variant_shape(kind: &ruxen_core::hir::nodes::HirVariantKind) -> String {
