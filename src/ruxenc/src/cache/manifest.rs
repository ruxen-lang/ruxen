...
/// Magic bytes that start every cache artifact: `b"RUXC"` (Ruxen Cache).
pub const MAGIC: [u8; 4] = *b"RUXC";
...
/// `target/ruxen/incremental/manifest.bin` with postcard.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
mod tests {
    ...
    #[test]
    fn header_magic_is_ruxc() {
        let h = CacheHeader::current("x86_64-linux", "debug");
        assert_eq!(&h.magic, b"RUXC");
