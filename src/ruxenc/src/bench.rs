//! `ruxenc bench` / `ruxen bench` — microbenchmark harness.
//!
//! `bench <file.rx>` finds every `def bench_*(b: &var Bencher)` in the
//! file, appends a synthesised `def main` that runs each one against a fresh
//! `Bencher`, and compiles+runs the result. Pure-Ruxen harness — see
//! `library/std/bench/src/lib.rx`. No compiler-side parser/keyword work;
//! bench fns are identified by name convention (`bench_*`) per
//! feedback_pure_ruxen_first.md.
//!
...
pub fn run(args: &[String]) -> Result<(), String> {
    if args.is_empty() {
        return Err("Usage: bench <file.rx> [--filter <pat>] [--iter-hint <N>]".into());
    }
    ...
    let path = path.ok_or_else(|| "bench: missing <file.rx>".to_string())?;

    if !path.ends_with(".rx") {
        return Err(format!("bench: expected a .rx file, got: {}", path));
    }
    ...
    for item in &program.items {
        if let TopLevelItem::Function(f) = item {
            ...
            if f.name.starts_with("bench_") && f.params.len() == 1 {
                ...
                // a non-identifier through would become arbitrary Ruxen
                // injection. Re-validate at the trust boundary.
                if !is_valid_ruxen_identifier(&f.name) {
                    return Err(format!(
                        "bench: function name `{}` is not a valid Ruxen identifier; \
    ...
    synth.push_str("\n# ── ruxenc bench: synthesised runner main ────────────────────\n");
    ...
    // pipeline.
    let tmp_dir = std::env::temp_dir().join("ruxenc-bench");
    ...
    let stem = Path::new(&path)
        ...
        .and_then(|s| s.to_str())
        .unwrap_or("bench");
    let synth_path = tmp_dir.join(format!("{}.bench.rx", stem));
    ...
    // `RUXENC_BENCH_TIMEOUT_SECS` (default 60). 0 disables.
    let bench_timeout: u64 = std::env::var("RUXENC_BENCH_TIMEOUT_SECS")
        .ok()
            ...
            match child.try_wait() {
                ...
                Ok(None) => {
                    if start.elapsed().as_secs() >= bench_timeout {
                        ...
                        let _ = child.wait();
                        return Err(format!(
                            "bench: bench process pid={} exceeded RUXENC_BENCH_TIMEOUT_SECS={}s and was killed. \
...
/// parser bug.
fn is_valid_ruxen_identifier(s: &str) -> bool {
