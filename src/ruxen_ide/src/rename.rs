...
//! * Reject Ruxen keywords regardless of category.
...
//!   .rx file the bootstrap loader merged in isn't currently tracked
...
/// (Ruxen uses PascalCase variants per ruby-naming.spec.md).
...
/// Full Ruxen keyword list per the Wave-2 task brief. Kept here rather
/// than reaching into `ruxen_core::lexer` so the LSP never widens its
...
/// unresolved at HIR time per `project_ruxen_mir_two_dispatch_paths.md`).
fn def_id_for_node(
    node: &NodeAtPosition,
    program: &HirProgram,
    symbols: &SymbolTable,
...
///   bootstrap loader merged them from another `.rx` file — Wave 2
...
fn is_renamable_def(def_id: DefId, name: &str, span: &Span, source: &str) -> bool {
    ...
    // from stdlib `.rx` files (e.g. `puts` from io.rx) into the same
