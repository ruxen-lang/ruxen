...
mod tests {
    ...
    fn goto_def_for_builtin_puts_does_not_crash() {
        ...
        // on). Wave 2 (#06.8) moved `puts` to a `lib "ruxen_runtime"`
        // decl in `library/std/io/src/lib.rx`. The LSP `analyze` entry
        // point doesn't track the source file the bootstrap loader
        // pulled the decl from, so goto-def lands on the placeholder
        // URI with the line number from io.rx — neither cleanly
