...
fn diagnostics_for_type_error() {
    ...
    let source = "def main\n  let x: Int = \"hello\"\nend\n";
    let result = analyze(source);
    let uri = lsp_types::Url::parse("file:///test.rx").unwrap();
    let diagnostics = ruxen_ide::diagnostics::collect_diagnostics(&result, &uri);
    assert!(
...
fn hover_shows_inferred_type() {
    ...
    let hover = ruxen_ide::hover::hover_at(&result, pos);
...
fn goto_definition_finds_variable() {
    ...
    if let Some(pos) = source.rfind('x') {
        ...
        let location = ruxen_ide::goto_def::goto_definition(&result, lsp_pos);
...
fn semantic_tokens_produces_output() {
    let source = "def main\n  let x = 42\nend\n";
    let result = analyze(source);
    let tokens = ruxen_ide::semantic_tokens::semantic_tokens(&result);
...
fn analysis_of_sample_program() {
    let source =
        std::fs::read_to_string("../../compiler/ruxen_core/tests/fixtures/sample_program.rx")
    ...
    let uri = lsp_types::Url::parse("file:///test.rx").unwrap();
    let _diagnostics = ruxen_ide::diagnostics::collect_diagnostics(&result, &uri);
}
...
fn e2e_hover_and_goto_consistent_for_variable() {
    ...
    let hover = ruxen_ide::hover::hover_at(&result, pos);
    let goto = ruxen_ide::goto_def::goto_definition(&result, pos);
...
fn e2e_semantic_tokens_and_diagnostics_on_type_error() {
    ...
    let source = "def main\n  let x: Int = \"oops\"\nend\n";
    let result = analyze(source);
    let uri = lsp_types::Url::parse("file:///bad.rx").unwrap();
    let diagnostics = ruxen_ide::diagnostics::collect_diagnostics(&result, &uri);
    assert!(
        !diagnostics.is_empty(),
...
fn e2e_goto_def_for_named_function() {
    ...
    let pos = result.line_index.position_of(offset);
    let loc = ruxen_ide::goto_def::goto_definition(&result, pos);
    assert!(loc.is_some());
...
fn e2e_hover_on_multiple_functions() {
    ...
    let h1 = ruxen_ide::hover::hover_at(&result, pos1);
    ...
    let h2 = ruxen_ide::hover::hover_at(&result, pos2);
...
fn e2e_diagnostics_preserve_positions_across_pipeline() {
    let source = "def main\n  let x: Int = \"bad\"\nend\n";
    let result = analyze(source);
    let uri = lsp_types::Url::parse("file:///t.rx").unwrap();
    let diagnostics = ruxen_ide::diagnostics::collect_diagnostics(&result, &uri);
...
fn e2e_semantic_tokens_are_nonoverlapping() {
    ...
    for t in &tokens {
        ...
        assert!(t.length > 0);
        // token_type is valid
        assert!((t.token_type as usize) < ruxen_ide::semantic_tokens::TOKEN_TYPES.len());
    }
...
fn e2e_diagnostic_collection_is_stable() {
    ...
    let result = analyze(source);
    let uri = lsp_types::Url::parse("file:///t.rx").unwrap();
    let diagnostics1 = ruxen_ide::diagnostics::collect_diagnostics(&result, &uri);
    let diagnostics2 = ruxen_ide::diagnostics::collect_diagnostics(&result, &uri);
