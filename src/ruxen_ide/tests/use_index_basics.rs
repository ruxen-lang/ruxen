...
//! lives in a `.rx` fixture under `tests/fixtures/use_index/`. Each
...
use ruxen_core::hir::nodes::DefId;
use ruxen_ide::analysis::{analyze, AnalysisResult};
use ruxen_ide::use_index::UseIndex;
...
/// (`project_ruxen_bootstrap_merge_skips_class_bodies.md`) sometimes
...
fn first_entry_is_the_definition_site() {
    ...
    // Ruxen's lexer is 1-indexed, so the first source line is `line == 1`.
    assert_eq!(
        spans[0].line, 1,
