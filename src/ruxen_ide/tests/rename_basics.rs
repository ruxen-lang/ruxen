...
//!
//! Ruxen source lives in `.rx` fixtures under `tests/fixtures/rename/`
//! (per `feedback_no_inline_rx_in_pin_tests.md`) using unique anchor
...
use ruxen_ide::rename::{prepare_rename, rename};
...
#[test]
fn rename_rejects_ruxen_keyword_as_new_name() {
    let src = load("local_var");
    let result = analyze(&src);
    let pos = pos_inside(&src, "tally_anchor_kzx", 1);
...
fn rename_on_builtin_puts_returns_none() {
    ...
    // `library/std/io/src/lib.rx` decls but `analyze` doesn't track
