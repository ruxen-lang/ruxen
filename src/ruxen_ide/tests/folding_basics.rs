...
//! lives in a `.rx` fixture under `tests/fixtures/folding/`. Each
...
use ruxen_ide::folding::folding_ranges;
...
fn def_body_folds_from_def_to_end() {
    // def_body.rx: `def fold_anchor_simple_def` on line 0, `end` on line 5.
...
fn class_with_methods_yields_multiple_ranges() {
    // class_with_methods.rx: class on lines 0..=12, three method defs inside.
...
fn if_elsif_else_chain_folds() {
    // if_elsif_else.rx: def + nested if/elsif/else. The outer def
...
#[test]
fn loop_body_folds() {
    // loop_body.rx: `def` on line 0, `loop` opens on line 2, `end`
...
fn consecutive_use_imports_collapse_into_one_range() {
    // uses_top_of_file.rx: three `use` lines (0..=2), blank, then def.
...
fn comment_block_of_three_lines_folds() {
    // comment_block.rx: lines 0..=2 are `#`-comments, then a def.
...
fn match_expression_folds() {
    // nested_match.rx: def on line 0, match on line 1 ending line 5,
    // def end on line 6.
