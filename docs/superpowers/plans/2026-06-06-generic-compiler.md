...
- [x] Step 1: after receiver-generic subst, for each UNBOUND method generic `P` on the `.rx` method, find `P` in the declared param types and structurally unify with the actual arg's inferred type. **FINDING: the harvest machinery was ALREADY complete** — `bind_type_params_from_args`' `any Fn[Fn(T)->U]` arm (closure-return) and its direct-arg/`Class`/`Enum` arms already bind a method generic; `harvest_and_subst_generics` already substitutes σ into the (receiver-substituted) return. No new harvest code was needed. The blocker was purely the *shadowing* `collections.rs` arms running ahead of the bridge.
- [x] Step 2: ordering — harvest runs AFTER the closure is inferred (`expr.rs` infers the block at the `infer_expr(blk)` call, then appends it to `effective_args` for `builtin_method_type`). Pin: `tests/release-e2e/cases/634_map_type_change_harvest` — type-CHANGING `map` chains (Int→String→USize) on both Option and Result, calling a `String`-only method on the mapped value so a broken harvest fails to type-check.
- [x] Step 3: deleted `collections.rs` arms — Option `map`, Result `map`/`map_err` (kept `try_op` + `ok_or`); deleted the now-redundant `expr.rs::infer_combinator_block`. The real signatures already lived in `option_result/lib.rx` (`map[U]`/`map_err[F]`). **`ok_or` NOT yet retired** — it's an FFI `lib` decl (`def ok_or as "ruxen_option_ok_or"(err: Int) -> Int`) with no example of a method-level generic on a `lib` decl; deferred to a follow-up increment (convert to a `.rx` body `match self { Some(x) -> Ok(x); nil -> Err(err) }` with `ok_or[E](err: E) -> Result[T, E]`, harvesting `E` from the direct arg like `zip[U]`).
- [x] Step 4: GATED — e2e all fixtures **341/0**; lib **564/0**; golden re-record clean (only the 3 intended rows `Option::map`/`Result::map`/`Result::map_err` removed, like `sum`). CLAUDE.md updated (`infer`/`method_resolvers`/`option_result`).

**STATUS: Increment 1 DONE (committed). Remaining: retire `ok_or` (the last harvestable Option/Result arm) — direct-arg `E` harvest, blocked only on confirming the parser accepts `[E]` on a `lib` decl OR converting `ok_or` to a `.rx` body.**
