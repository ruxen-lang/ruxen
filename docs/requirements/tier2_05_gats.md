...
Blocks: `LendingIterator`, `StreamingIterator`, `Pool` /
        `Collection` mixins that return borrowed views, any mixin with
...
- **G1.** `mixin T; type A[a]; type B[U, a]` — associated types
  with lifetime and type parameters.
- **G2.** `class S; include T; type A[a] = &a Str; type B[U, a] = Map[a, U]`
  — class bodies bind the generic associated type.
...
- **NG1.** GATs on mixins that are also used through `any`. See §7.3.
...
class WindowIter[T]
  ...
  pos: USize
  width: USize
  def init(@data: Array[T], @width: USize)
    self.pos = 0
  end
...
- The class body's `type Item[a] = ...` introduces `a` as a binder; the
...
- The body's method signature's `a` is the *same* lifetime as the
...
- `Self.Item[a]` inside a mixin body: projection with the mixin's
...
8. `LendingIterator` mixin in the prelude.
9. `Array.windows[N]`, `Array.chunks_mut` adapters.
...
GAT-using mixins are **not object-safe** unless every GAT is bound
...
Practically, `any LendingIterator` is forbidden because the vtable
...
### 7.6 With existential return types (doc 04)

`def iter[T] -> some LendingIterator[Item[a] = &a T]` — opaque type
...
- **R-2: error message quality.** GAT errors are the notorious
  ...
  formatter that points at both the mixin declaration and the class-body
  binding.
- **R-3: developer ergonomics.** Even Rust users struggle. Document
  prominently; provide worked examples in the tutorial. Consider
  auto-suggesting `where for[a] I.Item[a]: Mixin` when a naive
  bound fails.
...
- T1: `mixin LendingIterator` + class body `include` for `WindowIter[Int]`.
...
- T4: GAT with bound: `type Iter[a]: Iterator[Item = &a Self.Elem]`
  + class body satisfying the bound.
- T5: HRTB on GAT projection: `where for[a] W.Item[a]: Display`.

### 10.2 Negative tests

- N1: class body binds `type Item[a] = Array[T]` (drops `a`); bound
  `where for[a] I.Item[a]: Display` is fine. *Positive*. But
  GAT declaration that says `type Item[a]: Display` and a class-body
  binding `Array[T]` where `T` isn't `Display` → E-GAT-BOUND-VIOL.
- N2: Recursive GAT → E-GAT-CYCLE.
- N3: GAT with lifetime mismatch: class body declares `[a]` on the type but
  signature uses `[b]` → E-GAT-LIFETIME-UNBOUND (or alpha-rename
  silently — per OQ-1, alpha-rename).
- N4: `any LendingIterator` → E-ANY-NOT-SAFE (GAT makes the mixin
  non-object-safe unless bound).
- N5: Solver cycle depth exceeded (pathological mixin with
