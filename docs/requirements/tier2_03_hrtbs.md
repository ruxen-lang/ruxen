# Tier 2.03 — Higher-Ranked Mixin Bounds (HRTBs)
...
Blocks: fluent closure APIs that accept any input lifetime;
        `some Fn(&_ T) -> &_ T` style returns at the boundary
...
- **G1.** Functions and mixin bounds can be qualified by a universal
...
- **G4.** HRTBs on `any Mixin` are legal and produce the correct
  vtable (trivially — the vtable doesn't change; the type of the
  mixin existential does).
...
- **NG1.** Full predicative-polymorphic Rank-2 types (ML-style).
  HRTBs are restricted to mixin bounds; no `forall a. expr` at the
  term level.
...
- **NG5.** Higher-kinded types. `T[a]` as a parametric type
  constructor in a mixin bound. Out of scope entirely.
...
# Elided equivalent:
type LineProcessor = Box[any Fn(&Str) -> Result[(), Error]]
```
...
Legal. Rare because `some Mixin` in argument position (doc 04) is the
...
# Elaborated:
def each[F: for[a] Fn(&a Str)](items: &Array[String], f: F) ...
```
...
- If the method returns `some Iterator[Item = &T]`, the `&T` gets
...
pub struct TraitRef {
    ...
    pub higher_ranked: Vec<String>,    // NEW: [a, b] bound at this mixin ref
}
```

`higher_ranked` is empty for non-HRTB bounds. Non-empty: the listed
lifetime names are universally quantified over this specific mixin
reference.
...
`parse_single_mixin_bound` (`parser/types.rs:400-405`):
...
2. Implement mixin-existential closure elision (§5.2).
...
9. HRTBs on non-`Fn` mixins: `T: for[a] Iterator[Item = &a U]`.
   Requires GATs (doc 05) to express the quantified projection.
10. HRTBs with outlives bounds: `for[a: 'b] Fn(&a T)`.
11. `some Mixin` return with HRTB: `-> some for[a] Fn(&a T)`. Rare
...
### 8.3 With mixin existentials (doc 06)

`any for[a] Fn(&a T)` is object-safe iff `Fn` is object-safe
(it is — see doc 06 §4). The vtable layout is identical to `any Fn`;
...
`some for[a] Fn(&a T) -> U` in argument position is equivalent to
a named type param plus the HRTB bound. Elision covers the common
case. Explicit HRTBs on `some Mixin` are legal.

`some for[a] Fn(&a T) -> U` in return position is rare; reject in
...
- T1: `def each[F: Fn(&Str)](items: &Array[String], f: F)` — elided
...
- T4: `any Fn(&Str) -> Bool` accepts a mixin-existential closure without
  explicit lifetimes.
- T5: `Box[any for[a] Fn(&a Str) -> Bool]` stored in a field,
  invoked with a borrow.
- T6: HRTB on an `some Mixin` argument: `def each(items: &Array[String],
  f: some Fn(&Str))`.
