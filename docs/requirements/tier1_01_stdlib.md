...
- `Map` and `Set` are declared in the type system (`Ty::Map`, `Ty::Set` in `crates/riven-core/src/hir/types.rs:77-79`) but have **zero** runtime functions and **zero** typechecker method entries.
...
- No `panic!`, `println!`, `eprintln!`, `format!`, `dbg!` macros. `array!` exists (`crates/riven-core/src/mir/lower.rs:1502-1535`), `hash!` is documented (tutorial 13) but unimplemented.
...
`runtime_name()` maps mangled Riven method names (`Array[T]_push`, `String_trim`, `Option[T]_unwrap_or`, …) to C symbols. It has a large fallback block (lines 131-167) that maps unknown `?T…` (unresolved inference-variable) methods to best-effort runtime calls — this is how generic methods limp through today. The list `RUNTIME_FUNCTIONS` (lines 11-26) is out of date: it only names 14 of the ~25 functions actually in `runtime.c`.
...
7. FFI paths for `fs`, `env`, `process`, `time` use the `lib "<libname>" ... end` mechanism (see tutorial 14) through vetted wrappers — so `std.fs.read_to_string` is a Riven function that calls `fopen`/`fread`/`fclose` under the hood.
...
Every module below is a Riven module under `std`. Unless stated otherwise, items are public (sectionless top-level items are public by default). Names follow the conventions in `docs/tutorial/16`: `snake_case` for functions, `UpperCamelCase` for types, `SCREAMING_SNAKE_CASE` for constants.
...
- Macros: `println!`, `eprintln!`, `print!`, `eprint!`, `format!`, `panic!`, `dbg!`, `todo!`, `unimplemented!`, `assert!`, `assert_eq!`, `array!`, `hash!`, `set!`.
...
Every primitive (`Int`, `Float`, `Bool`, `Char`, `String`, `&str`), `Array`, `Map`, `Set`, `Option`, `Result`, `Tuple`, and fixed-size array types ship with blanket `Display` and/or `Debug` provisions. User types adopt `Display` via an `include Display` directive plus the required `fmt` method; `Debug` is auto-derivable (`derive Debug` on struct/class bodies, already parsed — see `StructDef::derive_traits` at `parser/ast.rs:597`).
...
Riven uses `.` as the path separator (`Http.Client`, `std.io.stdin`). The `package` keyword refers to the current compilation unit's root.
...
| `std.io` surface | Riven; bodies call into `lib "c"` libc bindings | already the FFI pattern (tutorial 14) |
| `std.fs` | Riven + `lib "c"` (`fopen`, `fread`, `fclose`, `unlink`, `mkdir`, `rename`, `stat`) | same |
| `std.net` | Riven + `lib "c"` (`socket`, `bind`, `listen`, `accept`, `connect`, `send`, `recv`, `close`) | same |
| `std.time` | Riven + `lib "c"` (`clock_gettime`) | same |
| `std.env` | Riven + `lib "c"` (`getenv`, `setenv`, `unsetenv`, `environ`) + compiler-emitted `argv` | argv needs main-shim support |
| `std.process` | Riven + `lib "c"` (`fork`, `execvp`, `waitpid`, `_exit`, `pipe`) | |
| `std.fmt` mixins | Riven | plain mixin defs |
...
| `std.hash` mixin | Riven | plain mixin |
| `std.hash.DefaultHasher` | Riven (SipHash provision) or `lib "c"` wrapping a bundled C SipHash | perf; either works |
...
Each macro call site becomes a `HirExprKind::Block` after expansion, with `HirExprKind::MacroCall` kept only as a fallback for `array!`/`hash!`/`set!` literal macros which already have a lowering path in `mir/lower.rs:1503`.
...
6. **MIR lowerer** (`mir/lower.rs`): add `hash!` / `set!` macro lowering next to the existing `array!` case at line 1502. Format-macro expansion is already handled at step 2 so no lowerer change.
...
- `Display` / `Debug` mixins with blanket provisions for primitives; `derive Debug` on structs/classes.
...
1. **`Hashable` vs hash-keyed map.** The canonical resolution chosen by spec §3.11 is:
   ...
   - The derive marker for `Hashable` is the in-body `derive Hash` directive, where `Hash` names the derive set, not a type.
...
7. **Tutorial drift.** The tutorial already promises `Hash.new` and `hash!{}`. The rename to `Map` requires the tutorial sweep tracked as a blocking subtask of phase 1a.
