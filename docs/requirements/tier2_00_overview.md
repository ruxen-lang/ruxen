...
| 03 | Higher-ranked mixin bounds (HRTBs) | [tier2_03_hrtbs.md](tier2_03_hrtbs.md) |
| 04 | `some Mixin` (argument/return) + specialization | [tier2_04_some_mixin_and_specialization.md](tier2_04_some_mixin_and_specialization.md) |
| 05 | Generic associated types (GATs) | [tier2_05_gats.md](tier2_05_gats.md) |
| 06 | Mixin existentials (`any Mixin`) | [tier2_06_any_mixin.md](tier2_06_any_mixin.md) |
...
1. **Mixin extensions that change `Ty` and `TraitRef`.** Associated types
   (01), GATs (05), HRTBs (03). These change what a mixin signature can
   mention and how the solver instantiates it.
2. **Dispatch and opacity.** `some Mixin` (return-position, argument-
   position) and specialization (04); mixin existentials with an object-safety
...
- **Type bodies accept `type Name = T` items.**
...
- **Bucket A (can ship without changing the runtime):** mixin existentials
...
- **Bucket B (requires real monomorphization — i.e., generating one
  ...
  path for a different `T`; `some Mixin` (04) in return position when
  the opaque type is not pointer-sized (e.g., `some Iterator` returning
  a struct).
...
- **Bucket C (ambiguous — depends on implementation detail):** HRTBs
  ...
  monomorphization; but HRTBs showing up in `some Mixin` on a struct
...
3. GATs,
4. specialization,
5. `some Mixin` in return position with non-pointer-sized opaque types.
...
tier-2 features that can ship are: mixin existentials (doc 06), variance
...
```
          ...
          ▼             ▼             ▼
     ┌─────────┐  ┌─────────┐  ┌──────────────┐
     │ M3  Assoc│  │ M4 Const│  │ M5  Mixin    │
     │   types  │  │ generics│  │   existentials│
     │  (01)    │  │ (02)    │  │    (06)      │
     └─────┬────┘  └─────────┘  └──────────────┘
           ▼
     ┌──────────────────────┐
...
- **Associated types (01) must come before Iterator-based stdlib.**
  `Array.iter` returns `some Iterator` whose `Item = &T`. Without
  `type Item` on the mixin, `Iterator.next` cannot be expressed,
...
- **Mixin existentials (06) are independent of monomorphization and can be
  shipped first.** `any Mixin` is a uniform fat pointer; the codegen
...
- **Specialization (04) is the highest-risk feature in the tier.**
  ...
  stabilised it after 7+ years of unsound interaction with mixins + Drop.
  **Recommendation: omit specialization from v1.** Ship `some Mixin`
...
- Tier-1 B1 (Drop codegen) must land before doc 01 phase 2 (associated
  ...
  projection inside an `Array[Item]` field leaks silently; with Drop, it
...
- Tier-1 doc 04 phase 4a (Drop mixin registration) is strictly
...
2. **Projection syntax: `Self.Item`.** Per the syntax spec, type-level
   ...
   `Identifier` — `lexer/mod.rs`. Docs 01, 05, 06 all use this form.
...
4. **Variance: inferred or declared?** Rust infers variance from
   ...
   surprising. Doc 07 §6 recommends inferred with an optional in-body
   `variance covariant` / `variance contravariant` directive as the
   escape hatch for documentation. Confirm.
...
- M5 mixin existentials: 2-3 weeks (independent; can run in parallel with M2)
- M6 HRTBs: 2 weeks
- M7 GATs: 3-4 weeks
- M8 some Mixin: 2 weeks
- M9 specialization: 4-6 weeks (recommended: skip)

Sequential critical path (excluding skip-ables): **~24-31 weeks.**
With M5 (mixin existentials) and M1 (variance) parallelised against M2, the
