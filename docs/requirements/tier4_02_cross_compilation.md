...
### 2.1 Cranelift backend (`crates/ruxen-core/src/codegen/cranelift.rs:41-75`)
...
### 2.2 LLVM backend (`crates/ruxen-core/src/codegen/llvm/mod.rs:37-83`)
...
### 2.3 Linking (`crates/ruxen-core/src/codegen/object.rs:10-92`)
...
### 2.4 Runtime (`crates/ruxen-core/runtime/runtime.c`, 426 lines)
...
`crates/ruxen-core/Cargo.toml:22` already declares `target-lexicon = "0.13"`. The type is used in Cranelift's internal dependencies but not referenced from Ruxen source. This gives us a tested, zero-new-dep parser for the `arch-vendor-os[-env]` triple grammar.
...
But this is the Rust toolchain cross-compiling the Ruxen *compiler binaries* — not Ruxen cross-compiling a user's project. The `cross` crate (line 53-54) is used only for the aarch64-Linux Rust build. Nothing in `ruxen-core` or `ruxen-cli` touches a triple.
...
- `.github/workflows/release.yml:79-103` creates `ruxen-${TAG}-${TARGET}.tar.gz`.
...
1. `--target <triple>` flag on `ruxen build`, `ruxen run`, `ruxen check`, and `ruxenc`.
...
4. Linker selection per target: `cc`, `aarch64-linux-gnu-gcc`, `wasm-ld` via an override table in `Ruxen.toml`.
5. Sysroot layout: precompiled `runtime.o` per target, shipped or fetched via `ruxen target add <triple>`.
...
- Running the cross-compiled binary on the host (QEMU / Rosetta integration). `ruxen run --target foo` errors cleanly.
...
- A full target-triple registry like `rustup target list`. Ship with a fixed set (§4.3) and let the user declare arbitrary triples in `Ruxen.toml` at their own risk.
- Cross-language linking (`ruxen-cc` wrapping a cross gcc). Users install cross toolchains themselves.
...
```
ruxen build  [--target <triple>] [--release] [--features ...] ...
ruxen run    [--target <triple>] [--release] ...         # errors if target != host
ruxen check  [--target <triple>] ...
ruxenc       [--target <triple>] [--backend=cranelift|llvm] ...

ruxen target list                                         # lists installed target runtimes
ruxen target list --all                                   # lists available targets (from the registry)
ruxen target add <triple>                                 # fetches & installs pre-built runtime.o
ruxen target remove <triple>
```
...
runner = "qemu-x86_64"                                       # optional — used by `ruxen run` if target != host
...
The `runner` field mirrors Cargo's `cargo run --target` with `CARGO_TARGET_<TRIPLE>_RUNNER`. If present, `ruxen run --target <triple>` invokes the runner with the built binary as an argument.
...
└── toolchains/
    ├── x86_64-unknown-linux-gnu/              # optional — cross toolchain fetched by `ruxen target add`
    │   └── bin/{gcc,ld,ar,...}
    └── ...
...
```
CLI (--target)   OR   Ruxen.toml ([build] default-target)   OR   $RUXEN_TARGET   OR   host triple
       │                                                                                   │
       └───────────────────────────┬───────────────────────────────────────────────────────┘
                                   ▼
...
New `crates/ruxen-cli/src/cfg.rs` — also referenced by doc 01 §5.2 for feature gating. Evaluation inputs:
...
fn linker_for(triple: &Triple, manifest: &Manifest) -> Result<(String, Vec<String>), String> {
    ...
    // 2. Environment override: RUXEN_TARGET_<TRIPLE_UPPER_UNDERSCORED>_LINKER
    let env_key = format!("RUXEN_TARGET_{}_LINKER", triple.to_string().to_uppercase().replace('-', "_"));
    ...
    Ok(match (triple.architecture, &triple.operating_system) {
        ...
        _ => return Err(format!("no default linker known for target '{}'. Set [target.<triple>].linker in Ruxen.toml.", triple)),
    })
}
```
...
1. `$RUXEN_RUNTIME_OBJ` (explicit override, path to `.o`).
2. `<exe>/../lib/runtime/<triple>/runtime.o` (installed sysroot).
3. `$RUXEN_HOME/lib/runtime/<triple>/runtime.o`.
...
If the runtime is missing and the user hasn't run `ruxen target add <triple>`, emit:

```
error: runtime for target 'aarch64-unknown-linux-gnu' not installed.
       Run `ruxen target add aarch64-unknown-linux-gnu` to download it,
       or set RUXEN_RUNTIME_OBJ to a path you built yourself.
```
...
### 5.9 `ruxen target add` implementation

A small HTTP client that fetches `https://releases.ruxen.land/<compiler-version>/runtime-<triple>.tar.gz`, verifies a sha256, extracts to `~/.ruxen/lib/runtime/<triple>/`.

The release workflow (`.github/workflows/release.yml`) grows a matrix job per tier-1 target that compiles `runtime.c` with the matching cross toolchain and uploads `runtime-<triple>.tar.gz` alongside the existing `ruxen-<tag>-<triple>.tar.gz` assets.

## 6. Implementation Plan — files to touch

### New files

- `crates/ruxen-cli/src/target.rs` — `ruxen target list/add/remove` subcommands.
- `crates/ruxen-cli/src/cfg.rs` — cfg parser + evaluator (shared with doc 01).
- `crates/ruxen-core/src/codegen/target.rs` — `Target` struct, triple parsing, linker-selection helper.

### Touched files

- `crates/ruxen-cli/src/cli.rs:24-114` — add `--target <triple>` to `Build`, `Run`, `Check`; add `Target { ... }` subcommand.
- `crates/ruxen-cli/src/manifest.rs:77-91` — add `[target.<triple>] { linker, link-args, runner, dependencies }` via `BTreeMap<String, TargetSpec>`.
- `crates/ruxen-cli/src/build.rs:21-118` — thread `Option<Triple>` through `build()`. Re-route `target_dir`.
- `crates/ruxen-core/src/codegen/mod.rs` — `Backend` variants gain target info; `compile_with_options` accepts `Triple`.
- `crates/ruxen-core/src/codegen/cranelift.rs:41-75` — `CodeGen::new(triple)` — swap `cranelift_native::builder` for `isa::lookup(triple)`.
- `crates/ruxen-core/src/codegen/llvm/mod.rs:37-83` — `CodeGen::new(opt_level, triple)` — swap `get_default_triple()` for the provided one.
- `crates/ruxen-core/src/codegen/object.rs:10-92` — accept `(Triple, &TargetSpec)`; pick linker; drop `-lc -lm` for wasm; thread link-args.
- `crates/ruxen-core/src/codegen/mod.rs:27-65` — add `find_runtime_obj(triple)` next to `find_runtime_c`.
...
- `crates/ruxen-cli/tests/cross_compile.rs` — builds for `aarch64-unknown-linux-gnu` from an x86_64 host, verifies the ELF header is `EM_AARCH64`.
- `crates/ruxen-core/tests/triple_parsing.rs` — alias resolution, canonicalization, cfg derivation.
- `crates/ruxen-core/src/codegen/tests.rs` — round-trip a simple program through both backends with `Triple::host()` and compare to the current behavior (regression guard).

## 7. Interactions with Other Tiers

- **Tier 4.01 package manager.** `[target.<triple>.dependencies]` lives in the manifest and is druxen by the resolved triple. `cfg(...)` evaluation in doc 01 §5.2 consumes the `CfgContext` built here.
...
- **Tier 4.06 CI.** The CI matrix needs to smoke-test cross-compilation. Recommend `aarch64-unknown-linux-gnu` from Ubuntu runners using `qemu-user-static` for `ruxen run` to succeed.
...
6. **Exit:** `ruxen build --target x86_64-unknown-linux-gnu` on a Linux host produces a bit-identical binary to `ruxen build`.

### Phase 2b — Cross-compile to non-host Linux (1 week)

1. Linker-selection table (§5.5).
2. `ruxen target add <triple>` stub that expects the user to have a cross cc (v1: no auto-fetch; point at README).
3. `aarch64-unknown-linux-gnu` smoke test in CI with `qemu-user-static`.
4. **Exit:** `ruxen build --target aarch64-unknown-linux-gnu` on x86_64 Ubuntu produces an aarch64 ELF that runs under QEMU.

### Phase 2c — Runtime object fetching (1 week)

1. Release workflow produces `runtime-<triple>.tar.gz` artifacts per tier-1 target.
2. `ruxen target add` fetches from `https://releases.ruxen.land/<version>/runtime-<triple>.tar.gz`.
3. `find_runtime_obj` prefers precompiled over source.
4. **Exit:** `ruxen target add aarch64-unknown-linux-gnu` on a fresh machine (no cross cc installed) succeeds. Subsequent `ruxen build --target …` uses the fetched runtime.
...
6. **`runner` for non-native `ruxen run`.** If target != host and no runner is configured, we error. Should we auto-detect QEMU? Recommend: no, too magical. Require explicit config.
...
8. **LLVM 18 vs 19.** `ruxen-core/Cargo.toml:27` features `llvm18-0`. LLVM 19 is out. Defer upgrade; LLVM 18 covers every target we name.
...
10. **Symbol naming differences.** macOS prepends `_` to C symbols; ELF does not. Cranelift and LLVM both handle this when given the right triple, but it's a place bugs hide. Regression test: same Ruxen source compiled for both Mach-O and ELF, disassembled, confirm `_ruxen_main` on Mach-O and `ruxen_main` on ELF.
11. **Environment variables leaking.** `CC`, `CFLAGS`, `LDFLAGS` — Cargo respects these with layered overrides. Recommend: Ruxen v1 ignores them (manifest + CLI only). Revisit if it causes friction.
12. **Ruxen-built `.rlib` portability.** A `.rlib` built for x86_64 is useless on aarch64. The lock file's checksum is *source*, not binary — so re-builds for a new target re-compile. `target/<triple>/deps/` isolates this.
