...
`riven test` runs functions matching `def test_*(t: &var Tester)` in a
...
| Aspect | Choice |
|---|---|
| CLI entry | `riven test` (subcommand on the unified `riven` binary). The `Test` variant already exists in `src/riven_cli/src/cli.rs` and exits 2 with a pointer to this doc until shipped. |
| Test dir | `test/` — any `.rvn` file under it. Repo-root + per-package. |
| Test fn shape | `def test_*(t: &var Tester)` — name-prefix convention, matches `bench_*` discovery so we reuse the bench machinery. |
| Helper class | `Tester` — parallel to `Bencher`. Lives in `library/std/test/src/lib.rvn`. |
...
| Discovery | Walk `test/**/*.rvn` from cwd; collect `def test_*` fns; synth a `def main` that runs each with a fresh `Tester`; aggregate pass/fail. Mirrors `rivenc bench`'s synth-main approach exactly. |
...
| Filter | `riven test --filter <pat>` — substring match on test fn name. Same shape as bench. |

## Where things live (precedent)

The pure-Riven `Bencher` pattern is the precedent. Mirror it:

- `library/std/bench/src/lib.rvn` → `library/std/test/src/lib.rvn`
- `src/rivenc/src/bench.rs` (lib fn `bench::run`) → `src/rivenc/src/test.rs` (lib fn `test::run`)
- `riven_cli::Command::Bench` → `riven_cli::Command::Test`
...
If/when Riven gains a macro system, the bang-form assertions can be added as sugar over the builder methods without churning the runtime.

## TDD plan (when the implementer picks this up)

- Unit test: a project with 3 tests (1 pass, 1 fail) runs via `riven test` and prints expected report.
...
2. Add `src/rivenc/src/test.rs` with `pub fn run(args: &[String]) -> Result<(), String>`.
3. Replace the `cli::Command::Test` stub in `src/riven_cli/src/main.rs` with a real dispatch into `rivenc::test::run`.
4. Output format: same shape as `cargo test` — `running N tests`, per-test `ok`/`FAILED` lines, summary footer.

## Definition of done

- [ ] `library/std/test/src/lib.rvn` defines `Tester` + `Expect`.
- [ ] `riven test` works on a fixture project (3 tests, mixed pass/fail).
