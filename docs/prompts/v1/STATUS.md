...
| `cargo test -p ruxen_core --tests` (macOS) | 9 failing | **1066 / 1066** |
| `cargo test --workspace` (linux, gcc) | red on build + 2 test failures | **1643 / 1643** |
| `cargo build --workspace --all-targets` | red (`ruxen_repl` non-exhaustive match + `Future_dynamic_poll` link miss) | **green** |
...
- `resolve/items.rs:332` — `resolve_class` now preserves
  ...
  `project_ruxen_resolve_class_stomps_typealias.md` at the write
...
- `library/std/{core,hash}/src/lib.rx` — `Hashable.hash_code`,
...
- `ruxen_repl/src/jit.rs` — `MirInst::DataAddr` arm added; non-
  exhaustive match was breaking `cargo build --workspace --all-targets`.
- `ruxen_repl/{build.rs,runtime_stubs.c}` — weak
  `Future_dynamic_poll` stub satisfies the libruxenrt link without
  changing AOT codegen behaviour.
- `ruxen_ide/src/line_index.rs` — UTF-8 char-boundary safety via a
  stable `floor_char_boundary` helper. Closes the v1.1 follow-up
  noted in the previous audit (`analysis_of_sample_program` was
  panicking on the `─` banner chars in `sample_program.rx`).

Two stale-test sweeps:

- `Task` → `Todo` rename across `compiler/ruxen_core/tests/fixtures/{class_methods,mini_sample,sample_program}.rx`
  (collided with the stdlib's async `class Task`).
- E1100 / E1101 / E1102 / E1110 / E1112 / E1115 / E1116 / E1117 /
  E1118 added to the `EXPLAINS` table in `ruxen_cli/src/explain.rs`
...
|---|---|---|
| 01 | Phase 1 remainder | Drop infra, structural mixins, error code registry — all in `compiler/ruxen_core/src/{mir/lower/drops.rs,implicit_includes/,diagnostics/codes.rs}` |
...
| 06 | stdlib io/fmt | `library/std/{io,fmt,fs,env,process,path,bufio}/`; ExitStatus + Metadata accessors moved C→Ruxen via `layout c` this session |
...
| 06.95 | stdlib packagization | Per-package `library/std/<pkg>/{src,runtime,Ruxen.toml}` for 25 packages; `auto_populate_std_submodules_from_packages` derives `std.<pkg>` items from BOOTSTRAP_FILES |
...
| 13 | benchmarking | `library/std/bench/` pure-Ruxen `Bencher` + `ruxenc bench` subcommand + 5 criterion compile-pipeline benches. JSON/baseline/MAD deferred to v1.5 |
| 14 | concurrency | Thread, Mutex, MutexGuard, SharedSync, Atomic*, Sender/Receiver, JoinHandle. Ruxen-level Mutex bench at `tests/benches/sync_mutex.rx` runs at **15 ns/iter** — 70× under the 1 µs/op DoD budget (100k ops in 1.5 ms vs the 100 ms target) |
...
| 11 | incremental | 🟡 Partial | ruxenc cache + `--force` + `clean` subcommand work; formal query-layer design TODO |
...
| 19 | test framework | ⬜ Not started | Tests are Rust-side over `.rx` fixtures; no Ruxen-native runner |
| 20 | ruxendoc | ⬜ Not started | Only spec in `docs/requirements/tier3_04_doc_generator.md` |
| 21 | MIR optimizer | ⬜ Not started | MIR has lowering, no opt pass tree |
| 22 | pkg manager / workspaces | 🟡 Partial | Per-package Ruxen.toml shipped; workspace/resolver/lockfile/registry TODO |
| 23 | language reference | ⬜ Not started | Spec-druxen `docs/specs/` exist; unified manual not assembled |
...
- ⬜ **Genuinely TODO**: 12 diagnostics polish, 16 no_std/wasm, 17 cross-compile, 18 debugger/DWARF, 19 test framework, 20 ruxendoc, 21 MIR optimizer, 23 language reference, 24 editions, 25 release checklist

**Closest-to-shipping next chunks:**
1. **19 test framework** — biggest user-facing v1 gap; unlocks user-side `def test_*` runner. Same pattern as prompt 13's `ruxenc bench` (pure Ruxen harness + tiny Rust CLI wrapper)
...
- Quality review Wave 2 (security): four CVE-class holes in `src/ruxen_cli/src/resolve_deps.rs` (argument injection on `git clone`, path-dep traversal, `dlsym(RTLD_DEFAULT)` open allowlist). Pre-1.0 is the right time.
