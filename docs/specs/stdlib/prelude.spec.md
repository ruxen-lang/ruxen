...
| `Vec[T]`        | `std::vec`         |
| `HashMap[K, V]` (alias `Hash[K, V]`) | `std::collections` |
...
| `FmtError`      | `std::fmt`          |
| `Formatter`     | `std::fmt`          |
| `Display`, `Debug`, `PartialEq`, `Eq`, `Hash`, `Clone`, `Copy`, `Default`, `Ord`, `PartialOrd`, `Drop`, `Iterator`, `FromIterator` | trait traits |
...
needing `use std.option.Option`.  Bare `Some(x)` / `None` / `Ok(x)` /
...
|-----------|-----------------------------------------------------------|----------------------------|
| B1, B3    | every release-e2e fixture that uses `Vec` / `Option` / `Result` without `use` (hundreds) | `tests/release-e2e/cases/` |
...
- Per-crate prelude (Rust's `core::prelude` vs `std::prelude`).
- Glob re-exports inside the prelude (`pub use std.io.*`).
