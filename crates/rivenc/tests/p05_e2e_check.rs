...
//! `rivenc` binary and report compile-fail / output-mismatch fixtures.
//!
...
#[test]
#[ignore] // run explicitly: `cargo test -p rivenc --test p05_e2e_check -- --ignored --nocapture`
fn release_e2e_fixtures_compile_and_match() {
    let manifest = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let workspace = manifest.parent().unwrap().parent().unwrap();
    let rivenc = workspace.join("target").join("release").join("rivenc");
    assert!(
        rivenc.exists(),
        "rivenc release binary not built at {}",
        rivenc.display()
    );
    ...
    let mut entries: Vec<_> = std::fs::read_dir(&cases)
        .unwrap()
        .filter_map(|e| e.ok())
        .filter(|e| e.path().extension().map(|x| x == "rvn").unwrap_or(false))
        .collect();
