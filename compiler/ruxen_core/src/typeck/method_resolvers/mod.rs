...
pub(super) fn builtin_method_type(
    ...
    match (ty, method) {
        ...
        //
        // `.new` defaults to "constructor yields Self", but a class may
        // declare its OWN `new` — most importantly an FFI
        // `def self.new ... -> Result[...]` such as `Regex.new`. Honour
        // that declared return type; otherwise the scrutinee of
        // `match Regex.new(...)` is typed `Regex` (not `Result`), the
        // `Ok(rx)` binding never resolves to a concrete type, and
        // `rx.is_match(...)` mangles to `?T<n>_is_match` with no runtime
        // symbol (codegen failure). Classes built via a plain `def init`
        // have no `new` method symbol, so the lookup returns None and the
        // constructor still yields Self. Concurrency types (Mutex / Arc /
        // SharedSync) match their own `new` arms above, so this never
        // shadows their Send-bound (E1101/E1102) checks.
        (Ty::Class { name, .. }, "new") => match eng.lookup_class_method_return(name, "new") {
            Some(ret) => Some(ret),
            None => Some(ty.clone()),
        },
