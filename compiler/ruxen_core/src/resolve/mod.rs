//! Name resolution pass for the Ruxen compiler.
//!
...
pub struct Resolver {
    ...
    /// no-op redundancy, not a conflict. The Ruxen-side name is
    ...
    /// same name (e.g. `def sleep` in both `library/std/time/src/lib.rx`
    /// and `library/std/sync/src/lib.rx`'s back-compat shim), the
    /// last-loaded package wins the lookup. The `std.<pkg>` submodule
    /// items list MUST contain the DefId of the item declared in
    /// `library/std/<pkg>/src/lib.rx`, not whoever-came-last — otherwise
    /// `use std.time.sleep` resolves to a different function than the
    /// one declared in `time/lib.rx`, and the signature mismatch
...
mod tests {
    ...
    fn register_builtins_includes_send_and_sync_traits() {
        ...
        // `library/std/core/src/lib.rx` (`mixin Send` / `mixin Sync`)
    ...
    /// bootstrap merge (self-hosted `library/std/<pkg>/src/lib.rx`). Tests
    ...
    fn register_builtins_includes_async_core_symbols() {
        ...
        // Context / Waker migrated to library/std/sync/src/lib.rx in
    ...
    fn register_builtins_includes_concurrency_core_symbols() {
        ...
        // `library/std/sync/src/lib.rx` (Wave 2, #06.8). The
        ...
        // sync.rx, not through a Class entry in the symbol table.
