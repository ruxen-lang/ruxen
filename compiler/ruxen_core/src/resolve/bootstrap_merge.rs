...
impl Resolver {
    ...
    /// parsed `.rx` file from `library/std/<pkg>/src/lib.rx`.
    ///
    ...
    /// with actual Ruxen function bodies arrives.
    ...
    pub fn resolve_with_bootstrap(
        ...
        // stdlib `.rx` class actually execute. Pre-B1 the merge only
    ...
    /// defined a name (e.g. `class File`) that a bootstrap `.rx`
    ...
    /// classes move from Rust registrations to `.rx` files, the
    /// `.rx` version naturally wins without flag-day coordination.
    /// Once a class's full surface is migrated, the corresponding
    /// `register_builtins` block can be deleted; the `.rx` version
    ...
    pub fn merge_bootstrap_programs(
        ...
        for (idx, program) in programs.iter().enumerate() {
            ...
            // `library/std/sync/src/lib.rx`'s back-compat shim
            // overwriting the typed `def sleep(d: &Duration) ->
            // TimeSleepFuture` from `library/std/time/src/lib.rx`)
    ...
    /// from the .rx files themselves: every migration commit
    /// (`stdlib(<module>): migrate from Rust registrations to Ruxen
    ...
    /// assembly — a `.rx`-defined `module rand … end` (or the
    ...
    fn auto_populate_std_submodules_from_packages(&mut self) {
        ...
        for (pkg_name, item_names) in pkgs.iter() {
            ...
            // name (`def sleep` in both time.rx and sync.rx), the
    ...
    pub(super) fn fixup_bootstrapped_stdlib_modules(&mut self) {
        ...
        // (stdlib-module-name, &[item-names-the-.rx-defines])
        //
    ...
    /// what is deferred.
    ///
    /// Wave 2 (#06.8): `Mixin` added to unblock `iter.rx` (Iterator /
    /// FromIterator), `hash.rx` (Hashable), and `fmt.rx`
