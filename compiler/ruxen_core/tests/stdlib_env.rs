...
//!
//! Strategy: compile a tiny Ruxen program that exercises the new
...
fn env_vars_snapshots_process_environment() {
    let source = rx("env_vars_snapshots_process_environment");
    let (stdout, stderr, ok) = compile_and_run_with_env(
        &source,
        "stdlib_env_vars_present",
        [("RUXEN_TEST_VARS_KEY", "expected-vars-value")],
    );
    assert!(
        ok,
...
fn env_vars_is_non_empty_when_one_var_set() {
    let source = rx("env_vars_is_non_empty_when_one_var_set");
...
fn env_current_dir_returns_ok_path() {
    let source = rx("env_current_dir_returns_ok_path");
    let (stdout, stderr, ok) = compile_and_run_with_env(
        &source,
...
fn env_var_returns_ok_for_set_key() {
    let source = rx("env_var_returns_ok_for_set_key");
    let (stdout, stderr, ok) =
        compile_and_run_with_env(&source, "stdlib_env_var_set", [("RUXEN_SENTINEL", "hello")]);
    assert!(ok, "stderr: {}", stderr);
...
fn env_var_returns_err_for_missing_key() {
    let source = rx("env_var_returns_err_for_missing_key");
    let (stdout, stderr, ok) = compile_and_run_with_env(
        &source,
...
fn env_args_includes_program_name() {
    let source = rx("env_args_includes_program_name");
    let (stdout, stderr, ok) = compile_and_run_with_env(
        &source,
