...
//! `.rx` source strings. No `ruxenc` subprocess is invoked, so each
...
//! release-e2e fixtures `tests/release-e2e/cases/60{3..}_iter_*.rx`
...
/// Ruxen source string. We deliberately stop at the in-memory object
...
fn iter_fold_compiles_with_int_accumulator() {
    let source = rx("iter_fold_compiles_with_int_accumulator");
    assert_compiles(&source);
}

#[test]
fn iter_all_compiles_returning_bool() {
    let source = rx("iter_all_compiles_returning_bool");
    assert_compiles(&source);
}

#[test]
fn iter_any_compiles_returning_bool() {
    let source = rx("iter_any_compiles_returning_bool");
    assert_compiles(&source);
}
...
fn iter_take_compiles() {
    let source = rx("iter_take_compiles");
    assert_compiles(&source);
}

#[test]
fn iter_skip_compiles() {
    let source = rx("iter_skip_compiles");
    assert_compiles(&source);
}

#[test]
fn iter_take_then_sum_compiles() {
    let source = rx("iter_take_then_sum_compiles");
    assert_compiles(&source);
}

#[test]
fn iter_skip_then_sum_compiles() {
    let source = rx("iter_skip_then_sum_compiles");
    assert_compiles(&source);
}
...
fn iter_enumerate_passthrough_compiles() {
    ...
    let source = rx("iter_enumerate_passthrough_compiles");
    assert_compiles(&source);
}

#[test]
fn iter_filter_then_count_compiles() {
    let source = rx("iter_filter_then_count_compiles");
    assert_compiles(&source);
}
...
fn iter_sum_still_compiles() {
    let source = rx("iter_sum_still_compiles");
    assert_compiles(&source);
}

#[test]
fn iter_count_still_compiles() {
    let source = rx("iter_count_still_compiles");
    assert_compiles(&source);
}
...
fn iter_skip_then_take_then_count_compiles() {
    let source = rx("iter_skip_then_take_then_count_compiles");
    assert_compiles(&source);
}
...
fn iter_take_then_fold_compiles() {
    ...
    let source = rx("iter_take_then_fold_compiles");
    assert_compiles(&source);
}
...
fn iter_skip_then_all_compiles() {
    ...
    let source = rx("iter_skip_then_all_compiles");
    assert_compiles(&source);
}
...
fn iter_chain_compiles() {
    ...
    // into a fresh `RuxenVec*` and continues the iter-style chain. The
    // surface type is the receiver's iter class so downstream
    // terminators (`sum`, `count`, `fold`, …) all keep working.
    let source = rx("iter_chain_compiles");
    assert_compiles(&source);
}

#[test]
fn iter_chain_then_sum_compiles() {
    let source = rx("iter_chain_then_sum_compiles");
    assert_compiles(&source);
}
...
fn iter_zip_then_count_compiles() {
    ...
    // fixture (607_iter_zip.rx).
    let source = rx("iter_zip_then_count_compiles");
    assert_compiles(&source);
}
...
fn iter_collect_string_compiles() {
    let source = rx("iter_collect_string_compiles");
    assert_compiles(&source);
}

#[test]
fn iter_collect_hashmap_compiles() {
    let source = rx("iter_collect_hashmap_compiles");
    assert_compiles(&source);
}

#[test]
fn iter_collect_hashset_compiles() {
    let source = rx("iter_collect_hashset_compiles");
    assert_compiles(&source);
}

#[test]
fn string_from_iter_compiles() {
    let source = rx("string_from_iter_compiles");
    assert_compiles(&source);
}

#[test]
fn hashmap_from_iter_compiles() {
    let source = rx("hashmap_from_iter_compiles");
    assert_compiles(&source);
}

#[test]
fn hashset_from_iter_compiles() {
    let source = rx("hashset_from_iter_compiles");
    assert_compiles(&source);
}
...
/// `*Iter.sum` to `ruxen_vec_sum` (which integer-sums the raw 64-bit
...
fn sum_on_string_iter_typeck_rejects() {
    let source = rx("sum_on_string_iter_typeck_rejects");
...
fn sum_on_int_iter_still_compiles_after_tightening() {
    let source = rx("sum_on_int_iter_still_compiles_after_tightening");
    let diags = typecheck_diagnostics(&source);
    let errors: Vec<&Diagnostic> = diags
        .iter()
        .filter(|d| d.level == DiagnosticLevel::Error)
