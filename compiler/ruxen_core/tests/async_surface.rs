...
//! `tests/release-e2e/cases/720_handwritten_future_typechecks.rx`
...
/// merge runs against the same `library/std/<pkg>/src/lib.rx`
...
fn future_mixin_has_associated_output_and_poll() {
    ...
    let future = symbols
        ...
        .expect("expected `Future` mixin from library/std/future/src/lib.rx");
...
/// `library/std/future/src/lib.rx`. Both variants are reachable as
...
fn poll_enum_registered_with_ready_pending_variants() {
    ...
    let poll = symbols
        .iter()
        .find(|d| d.name == "Poll")
        .expect("expected `Poll` enum from library/std/future/src/lib.rx");
...
/// was "Pending" (or vice versa). The test reads the .rx source
...
fn poll_tag_layout_stability() {
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .parent()
        .unwrap()
        .join("library/std/future/src/lib.rx");
    let source =
...
/// `library/std/future/src/lib.rx` as Class DefKinds. Sub-phase 1 ships
/// them with lib decls pointing at the stubbed executor (every method
/// `ruxen_panic`s); the contract pinned here is just that the class
...
fn context_and_waker_classes_resolve() {
    ...
    let waker = symbols
        ...
        .expect("expected `Waker` class from library/std/future/src/lib.rx");
    let _ = waker;
}
