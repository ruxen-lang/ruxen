...
/// Read a Ruxen fixture file from `tests/fixtures/ruxen/<name>.rx`.
...
fn parse_unsafe_block() {
    let source = rx("parse_unsafe_block");
...
fn parse_unsafe_block_with_statements() {
    let source = rx("parse_unsafe_block_with_statements");
    let program = parse(&source);
    assert!(!program.items.is_empty());
}
...
fn parse_null_literal() {
    let source = rx("parse_null_literal");
    let program = parse(&source);
    assert!(!program.items.is_empty());
...
fn parse_raw_pointer_type() {
    let source = rx("parse_raw_pointer_type");
    let program = parse(&source);
    assert!(!program.items.is_empty());
...
fn parse_raw_var_pointer_type() {
    let source = rx("parse_raw_var_pointer_type");
    let program = parse(&source);
    assert!(!program.items.is_empty());
...
fn parse_lib_block_with_link_name() {
    ...
    let source = rx("parse_lib_block_with_link_name");
    let program = parse(&source);
    assert!(!program.items.is_empty());
...
fn parse_lib_block_replaces_extern() {
    let source = rx("parse_lib_block_replaces_extern");
    let program = parse(&source);
    assert!(!program.items.is_empty());
...
fn parse_ffi_void_return() {
    let source = rx("parse_ffi_void_return");
    let program = parse(&source);
...
fn parse_ffi_multiple_params() {
    let source = rx("parse_ffi_multiple_params");
    let program = parse(&source);
...
fn parse_layout_c_struct() {
    ...
    let source = rx("parse_layout_c_struct");
    let program = parse(&source);
    assert!(!program.items.is_empty());
...
fn parse_include_on_struct() {
    let source = rx("parse_include_on_struct");
    let program = parse(&source);
    if let TopLevelItem::Struct(s) = &program.items[0] {
...
fn parse_include_on_class() {
    let source = rx("parse_include_on_class");
    let program = parse(&source);
...
fn parse_include_on_enum() {
    let source = rx("parse_include_on_enum");
    let program = parse(&source);
...
fn parse_inbody_include_on_class() {
    ...
    let source = rx("parse_inbody_include_on_class");
    let program = parse(&source);
    if let TopLevelItem::Class(c) = &program.items[0] {
        let traits: Vec<String> = c
            .inner_impls
...
fn packed_struct_layout() {
    use ruxen_core::codegen::layout::{packed_struct_layout, TypeLayout};
...
fn transparent_struct_layout() {
    use ruxen_core::codegen::layout::{transparent_struct_layout, TypeLayout};
...
fn c_struct_layout_matches_regular() {
    use ruxen_core::codegen::layout::{c_struct_layout, layout_struct_fields, TypeLayout};

    // C layout should match the regular struct layout
    // (since Ruxen already uses C-compatible layout rules).
    let fields = vec![
        TypeLayout {
            size: 1,
            alignment: 1,
