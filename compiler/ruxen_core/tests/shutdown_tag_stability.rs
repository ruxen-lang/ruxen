...
//! (`compiler/ruxen_core/src/resolve/stdlib/mod.rs`) co-document the
//! tag indices for each `Shutdown` variant. `ruxen_tcp_stream_shutdown`
...
fn shutdown_tag_values_match_runtime_and_stdlib_source() {
    ...
    // `compiler/ruxen_core/src/resolve/stdlib/mod.rs` to the
    // self-hosted `library/std/net/src/lib.rx`. The variant-tag
    // contract against `RUXEN_SHUTDOWN_*` in
    ...
    let stdlib_source = read("library/std/net/src/lib.rx");

    // Runtime side: scan for `#define RUXEN_SHUTDOWN_<NAME>  <tag>`.
    let mut runtime_tags: Vec<(String, usize)> = Vec::new();
    for line in runtime.lines() {
        let trimmed = line.trim();
        let prefix = "#define RUXEN_SHUTDOWN_";
    ...
    // order under `enum Shutdown ... end` in net/src/lib.rx. The variant's
    // tag is its zero-based position in that list — Ruxen's
    ...
    assert_eq!(
        runtime_tags.len(),
        3,
        "expected 3 RUXEN_SHUTDOWN_* defines in runtime; got {:?}",
        runtime_tags
    );
    assert_eq!(
        stdlib_tags.len(),
        3,
        "expected 3 Shutdown variants in library/std/net/src/lib.rx; got {:?}",
        stdlib_tags
    );
    assert_eq!(
...
#[test]
fn ruxen_tcp_listener_static_assert_is_eight_bytes() {
    let runtime = read("library/std/net/runtime/tcp.c");
    assert!(
        runtime.contains("_Static_assert(sizeof(RuxenTcpListener) == 8"),
        "expected `_Static_assert(sizeof(RuxenTcpListener) == 8 ...)` in tcp.c — \
...
#[test]
fn ruxen_tcp_stream_static_assert_is_eight_bytes() {
    let runtime = read("library/std/net/runtime/tcp.c");
    assert!(
        runtime.contains("_Static_assert(sizeof(RuxenTcpStream) == 8"),
        "expected `_Static_assert(sizeof(RuxenTcpStream) == 8 ...)` in tcp.c"
    );
}
