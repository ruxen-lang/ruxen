...
//! `tests/release-e2e/cases/50[1-9]_hashmap_*.rx` and the leak guard
...
fn hashmap_constructors_typecheck() {
    let source = rx("hashmap_constructors_typecheck");
...
fn hashmap_remove_returns_option() {
    let source = rx("hashmap_remove_returns_option");
    let diags = typecheck_diagnostics(&source);
    let errs = errors(&diags);
    assert!(
        errs.is_empty(),
...
fn hashmap_keys_values_iter_typecheck() {
    let source = rx("hashmap_keys_values_iter_typecheck");
    let diags = typecheck_diagnostics(&source);
    let errs = errors(&diags);
    assert!(
        errs.is_empty(),
...
fn hashmap_clear_typechecks_as_unit() {
    let source = rx("hashmap_clear_typechecks_as_unit");
    let diags = typecheck_diagnostics(&source);
    let errs = errors(&diags);
    assert!(
        errs.is_empty(),
...
/// `HashMap == HashMap` routes through the `ruxen_hash_eq` runtime
/// helper at MIR lowering. Pins the typeck contract: returns Bool.
#[test]
fn hashmap_equality_yields_bool() {
    let source = rx("hashmap_equality_yields_bool");
    let diags = typecheck_diagnostics(&source);
    let errs = errors(&diags);
    assert!(
        errs.is_empty(),
...
fn hashmap_entry_or_insert_typechecks() {
    let source = rx("hashmap_entry_or_insert_typechecks");
    let diags = typecheck_diagnostics(&source);
    let errs = errors(&diags);
    assert!(
        errs.is_empty(),
...
/// when the key is absent (the lazy-default contract). Ruxen uses a
/// trailing-block form for closure args (parser rejects `(|| 42)` as
/// an inline expression).
#[test]
fn hashmap_entry_or_insert_with_typechecks() {
    let source = rx("hashmap_entry_or_insert_with_typechecks");
    let diags = typecheck_diagnostics(&source);
    let errs = errors(&diags);
    assert!(
        errs.is_empty(),
