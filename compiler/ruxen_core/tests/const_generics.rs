...
use ruxen_core::parser::ast::{GenericParam, Program, TopLevelItem, TypeExpr};
...
fn parse_const_generic_param_on_struct() {
    let source = rx("parse_const_generic_param_on_struct");
...
fn parse_const_generic_param_on_class() {
    let source = rx("parse_const_generic_param_on_class");
    let prog = parse(&source);
...
fn parse_const_generic_param_on_fn() {
    let source = rx("parse_const_generic_param_on_fn");
    let prog = parse(&source);
...
fn parse_multiple_const_generic_params_typecheck_position() {
    let source = rx("parse_multiple_const_generic_params_typecheck_position");
    let prog = parse(&source);
...
fn parse_mixed_type_and_const_generic_params() {
    let source = rx("parse_mixed_type_and_const_generic_params");
    let prog = parse(&source);
...
fn into_named(ty: &TypeExpr) -> &ruxen_core::parser::ast::TypePath {
    match ty {
...
fn parse_const_lit_as_generic_arg() {
    let source = rx("parse_const_lit_as_generic_arg");
    let prog = parse(&source);
...
fn parse_multiple_const_lits_as_generic_args() {
    let source = rx("parse_multiple_const_lits_as_generic_args");
    let prog = parse(&source);
    let s = prog
        .items
        .iter()
...
fn const_generic_declarations_typecheck_clean() {
    let source = rx("const_generic_declarations_typecheck_clean");
    let mut lx = ruxen_core::lexer::Lexer::new(&source);
    let toks = lx.tokenize().expect("lex");
    let mut p = ruxen_core::parser::Parser::new(toks);
    let prog = p.parse().expect("parse");
    let result = ruxen_core::typeck::type_check(&prog);
...
fn const_expr_eval_lit_and_param() {
    use ruxen_core::hir::types::ConstExpr;
...
fn array_layout_evaluates_const_expr_lit() {
    use ruxen_core::codegen::layout::layout_of;
    use ruxen_core::hir::types::{ConstExpr, Ty};
    use ruxen_core::resolve::symbols::SymbolTable;
...
fn const_args_thread_into_class_generic_args() {
    ...
    let source = rx("const_args_thread_into_class_generic_args");
    let mut lx = ruxen_core::lexer::Lexer::new(&source);
    let toks = lx.tokenize().expect("lex");
    let mut p = ruxen_core::parser::Parser::new(toks);
    let prog = p.parse().expect("parse");
    ...
    let by_name = |target: &str| -> Option<Ty> {
        result.program.items.iter().find_map(|item| match item {
            ruxen_core::hir::nodes::HirItem::Function(f) if f.name == target => {
...
fn const_lit_against_type_param_emits_e0704() {
    let source = rx("const_lit_against_type_param_emits_e0704");
    let mut lx = ruxen_core::lexer::Lexer::new(&source);
    let toks = lx.tokenize().expect("lex");
    let mut p = ruxen_core::parser::Parser::new(toks);
    let prog = p.parse().expect("parse");
...
fn const_lit_against_const_param_typechecks_clean() {
    let source = rx("const_lit_against_const_param_typechecks_clean");
    let mut lx = ruxen_core::lexer::Lexer::new(&source);
    let toks = lx.tokenize().expect("lex");
    let mut p = ruxen_core::parser::Parser::new(toks);
    let prog = p.parse().expect("parse");
...
fn const_generic_param_registered_in_symbol_table() {
    ...
    let source = rx("const_generic_param_registered_in_symbol_table");
    let mut lx = ruxen_core::lexer::Lexer::new(&source);
    let toks = lx.tokenize().expect("lex");
    let mut p = ruxen_core::parser::Parser::new(toks);
    let prog = p.parse().expect("parse");
...
fn parse_const_lit_with_trailing_type_arg() {
    let source = rx("parse_const_lit_with_trailing_type_arg");
    let prog = parse(&source);
    let s = prog
        .items
        .iter()
...
fn const_expr_eval_op_basic_arithmetic() {
    use ruxen_core::hir::types::{ConstExpr, ConstOp};
    use std::collections::HashMap;
...
fn const_expr_eval_op_division_by_zero() {
    use ruxen_core::hir::types::{ConstEvalError, ConstExpr, ConstOp};
    use std::collections::HashMap;
...
fn array_layout_evaluates_const_expr_arithmetic() {
    ...
    use ruxen_core::codegen::layout::layout_of;
    use ruxen_core::hir::types::{ConstExpr, ConstOp, Ty};
    use ruxen_core::resolve::symbols::SymbolTable;
...
fn resolve_first_struct_field_ty(src: &str, struct_name: &str) -> ruxen_core::hir::types::Ty {
    use ruxen_core::hir::nodes::HirItem;
...
fn resolve_array_size_lowers_binary_add_to_const_op() {
    ...
    let source = rx("resolve_array_size_lowers_binary_add_to_const_op");
...
fn resolve_array_size_lowers_nested_arithmetic() {
    ...
    let source = rx("resolve_array_size_lowers_nested_arithmetic");
...
fn resolve_array_size_lowers_param_reference_in_arithmetic() {
    ...
    let source = rx("resolve_array_size_lowers_param_reference_in_arithmetic");
...
fn parse_const_arg_int_literal_at_call_site_is_type_application() {
    ...
    use ruxen_core::parser::ast::{ExprKind, Statement, TopLevelItem};
    let source = rx("parse_const_arg_int_literal_at_call_site_is_type_application");
    let prog = parse(&source);
    let main = prog
        .items
        .iter()
...
fn parse_const_arg_arithmetic_emits_const_expr_arg() {
    use ruxen_core::parser::ast::{BinOp, Expr, ExprKind, TopLevelItem, TypeExpr};
    let source = rx("parse_const_arg_arithmetic_emits_const_expr_arg");
    let prog = parse(&source);
    let s = prog
        .items
        .iter()
...
fn parse_const_arg_bare_literal_still_emits_const_lit() {
    ...
    let source = rx("parse_const_arg_bare_literal_still_emits_const_lit");
    let prog = parse(&source);
    let s = prog
        .items
        .iter()
...
fn resolve_const_arg_arithmetic_lowers_to_const_expr_op() {
    ...
    let source = rx("resolve_const_arg_arithmetic_lowers_to_const_expr_op");
    let mut lx = ruxen_core::lexer::Lexer::new(&source);
    let toks = lx.tokenize().expect("lex");
    let mut p = ruxen_core::parser::Parser::new(toks);
    let prog = p.parse().expect("parse");
...
fn resolve_normalises_array_size_n_plus_zero_equals_n() {
    ...
    let source = rx("resolve_normalises_array_size_n_plus_zero_equals_n");
    let mut lx = ruxen_core::lexer::Lexer::new(&source);
    let toks = lx.tokenize().expect("lex");
    let mut p = ruxen_core::parser::Parser::new(toks);
    let prog = p.parse().expect("parse");
...
fn resolve_normalises_const_arg_arithmetic_with_one_factor() {
    ...
    let source = rx("resolve_normalises_const_arg_arithmetic_with_one_factor");
    let mut lx = ruxen_core::lexer::Lexer::new(&source);
    let toks = lx.tokenize().expect("lex");
    let mut p = ruxen_core::parser::Parser::new(toks);
    let prog = p.parse().expect("parse");
...
fn layout_of_class_with_array_field_uses_const_arg_binding() {
    ...
    let source = rx("layout_of_class_with_array_field_uses_const_arg_binding");
    let mut lx = ruxen_core::lexer::Lexer::new(&source);
    let toks = lx.tokenize().expect("lex");
    let mut p = ruxen_core::parser::Parser::new(toks);
    let prog = p.parse().expect("parse");
...
fn layout_of_class_without_const_param_field_unchanged() {
    ...
    let source = rx("layout_of_class_without_const_param_field_unchanged");
    let mut lx = ruxen_core::lexer::Lexer::new(&source);
    let toks = lx.tokenize().expect("lex");
    let mut p = ruxen_core::parser::Parser::new(toks);
    let prog = p.parse().expect("parse");
...
fn bool_const_param_with_non_boolean_literal_emits_e0701() {
    ...
    let source = rx("bool_const_param_with_non_boolean_literal_emits_e0701");
...
fn bool_const_param_with_zero_or_one_typechecks_clean() {
    ...
    let template = rx("bool_const_param_with_zero_or_one_typechecks_clean");
    for value in [0, 1] {
...
fn usize_const_param_with_positive_literal_does_not_emit_e0701() {
    ...
    // common case).
    let source = rx("usize_const_param_with_positive_literal_does_not_emit_e0701");
    let codes = typecheck_diag_codes(&source);
    assert!(
        !codes.iter().any(|c| c == "E0701"),
...
fn where_n_gt_zero_at_zero_emits_e0706() {
    let source = rx("where_n_gt_zero_at_zero_emits_e0706");
    let codes = typecheck_diag_codes(&source);
    assert!(
...
fn where_n_gt_zero_at_positive_typechecks_clean() {
    let source = rx("where_n_gt_zero_at_positive_typechecks_clean");
    let codes = typecheck_diag_codes(&source);
    assert!(
        !codes.iter().any(|c| c == "E0706"),
...
fn where_n_eq_m_at_mismatched_values_emits_e0706() {
    ...
    let source = rx("where_n_eq_m_at_mismatched_values_emits_e0706");
    let codes = typecheck_diag_codes(&source);
    assert!(
        codes.iter().any(|c| c == "E0706"),
...
fn where_arithmetic_predicate_evaluates_at_instantiation() {
    ...
    let src_ok = rx("where_arithmetic_predicate_evaluates_at_instantiation_a");
    ...
    let src_bad = rx("where_arithmetic_predicate_evaluates_at_instantiation_b");
...
fn class_without_where_clause_is_unaffected() {
    let source = rx("class_without_where_clause_is_unaffected");
    let codes = typecheck_diag_codes(&source);
    assert!(
        !codes.iter().any(|c| c == "E0706"),
...
fn parse_where_clause_const_predicate_n_gt_zero() {
    use ruxen_core::parser::ast::{BinOp, ExprKind, TopLevelItem};
    let source = rx("parse_where_clause_const_predicate_n_gt_zero");
    let prog = parse(&source);
    let func = prog
        .items
        .iter()
...
fn parse_where_clause_const_predicate_n_eq_m() {
    use ruxen_core::parser::ast::{BinOp, ExprKind, TopLevelItem};
    let source = rx("parse_where_clause_const_predicate_n_eq_m");
    let prog = parse(&source);
    let func = prog
        .items
        .iter()
...
fn parse_where_clause_const_predicate_arithmetic_eq() {
    ...
    use ruxen_core::parser::ast::{BinOp, ExprKind, TopLevelItem};
    let source = rx("parse_where_clause_const_predicate_arithmetic_eq");
    let prog = parse(&source);
    let func = prog
        .items
        .iter()
...
fn parse_where_clause_mixed_trait_bound_and_const_predicate() {
    ...
    use ruxen_core::parser::ast::TopLevelItem;
    let source = rx("parse_where_clause_mixed_trait_bound_and_const_predicate");
    let prog = parse(&source);
    let func = prog
        .items
        .iter()
...
fn parse_where_clause_trait_bound_alone_still_works() {
    ...
    use ruxen_core::parser::ast::TopLevelItem;
    let source = rx("parse_where_clause_trait_bound_alone_still_works");
    let prog = parse(&source);
    let func = prog
        .items
        .iter()
...
fn const_param_float_type_emits_e0705() {
    let source = rx("const_param_float_type_emits_e0705");
    let codes = typecheck_diag_codes(&source);
    assert!(
...
fn const_param_string_type_emits_e0705() {
    let source = rx("const_param_string_type_emits_e0705");
    let codes = typecheck_diag_codes(&source);
    assert!(
        codes.iter().any(|c| c == "E0705"),
...
fn const_param_user_class_type_emits_e0705() {
    let source = rx("const_param_user_class_type_emits_e0705");
    let codes = typecheck_diag_codes(&source);
    assert!(
        codes.iter().any(|c| c == "E0705"),
...
fn const_param_bad_type_does_not_stack_on_unresolved_type() {
    ...
    let source = rx("const_param_bad_type_does_not_stack_on_unresolved_type");
    let codes = typecheck_diag_codes(&source);
    assert!(
        !codes.iter().any(|c| c == "E0705"),
...
fn array_size_overflow_emits_e0703() {
    ...
    let source = rx("array_size_overflow_emits_e0703");
    let codes = typecheck_diag_codes(&source);
    assert!(
...
fn array_size_division_by_zero_emits_e0703() {
    let source = rx("array_size_division_by_zero_emits_e0703");
    let codes = typecheck_diag_codes(&source);
    assert!(
        codes.iter().any(|c| c == "E0703"),
...
fn const_arg_position_overflow_emits_e0703() {
    let source = rx("const_arg_position_overflow_emits_e0703");
    let codes = typecheck_diag_codes(&source);
    assert!(
        codes.iter().any(|c| c == "E0703"),
...
fn array_size_param_arithmetic_does_not_emit_e0703() {
    ...
    let source = rx("array_size_param_arithmetic_does_not_emit_e0703");
    let codes = typecheck_diag_codes(&source);
    assert!(
        !codes.iter().any(|c| c == "E0703"),
...
fn array_size_bare_literal_does_not_emit_e0703() {
    ...
    let source = rx("array_size_bare_literal_does_not_emit_e0703");
    let codes = typecheck_diag_codes(&source);
    assert!(
        !codes.iter().any(|c| c == "E0703"),
...
fn array_size_clean_arithmetic_does_not_emit_e0703() {
    ...
    // `Ok(5)`, no diagnostic.
    let source = rx("array_size_clean_arithmetic_does_not_emit_e0703");
    let codes = typecheck_diag_codes(&source);
    assert!(
        !codes.iter().any(|c| c == "E0703"),
...
fn const_arg_arithmetic_against_type_param_emits_e0704() {
    ...
    // produces.
    let source = rx("const_arg_arithmetic_against_type_param_emits_e0704");
    let mut lx = ruxen_core::lexer::Lexer::new(&source);
    let toks = lx.tokenize().expect("lex");
    let mut p = ruxen_core::parser::Parser::new(toks);
    let prog = p.parse().expect("parse");
...
fn resolve_array_size_unsupported_op_becomes_error() {
    ...
    use ruxen_core::hir::types::{ConstEvalError, ConstExpr, Ty};

    let source = rx("resolve_array_size_unsupported_op_becomes_error");
    let ty = resolve_first_struct_field_ty(&source, "Buf");
    match ty {
        Ty::FixedArray(_, size) => {
...
fn array_size_unsupported_op_emits_e0702() {
    let source = rx("array_size_unsupported_op_emits_e0702");
    let codes = typecheck_diag_codes(&source);
    assert!(
...
fn array_size_comparison_op_emits_e0702() {
    ...
    let source = rx("array_size_comparison_op_emits_e0702");
    let codes = typecheck_diag_codes(&source);
    assert!(
        codes.iter().any(|c| c == "E0702"),
...
fn array_size_clean_arithmetic_does_not_emit_e0702() {
    ...
    let source = rx("array_size_clean_arithmetic_does_not_emit_e0702");
    let codes = typecheck_diag_codes(&source);
    assert!(
        !codes.iter().any(|c| c == "E0702"),
...
fn array_size_param_reference_does_not_emit_e0702() {
    ...
    let source = rx("array_size_param_reference_does_not_emit_e0702");
    let codes = typecheck_diag_codes(&source);
    assert!(
        !codes.iter().any(|c| c == "E0702"),
