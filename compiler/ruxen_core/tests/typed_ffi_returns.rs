...
//!
//! Fixtures live in `compiler/ruxen_core/tests/fixtures/ruxen/` per
//! the team's no-inline-rx-source rule. The end-to-end runtime
...
/// emit a `Mutex_drop` (ruxen_mutex_drop) call. The typeck-side lift
...
fn mutex_drop_emitted_on_typed_scope_exit() {
    let source = rx("typed_ffi_mutex_new_int");
    let mut lx = Lexer::new(&source);
    let toks = lx.tokenize().expect("lex");
    let mut p = Parser::new(toks);
    let prog = p.parse().expect("parse");
    ...
    // library/std/sync/src/lib.rx. Confirm it's registered in the
    // symbol table as a method (the `__drop` legacy name was retired
    // during the sync.rx rename — see
    // `project_ruxen_drop_name_mismatch.md`).
...
/// resolver for `Mutex` lists `ruxen_mutex_new` with a return type
...
fn ffi_signature_unchanged_after_lift() {
    ...
    let mutex_new = result
        ...
        .find(|f| f.c_symbol.as_deref() == Some("ruxen_mutex_new"))
        .expect("ruxen_mutex_new should be registered through the Mutex class lib decl");
    ...
    let ret_ty = mutex_new
        .return_type
        .as_ref()
        .expect("ruxen_mutex_new return type must be present");
    match ret_ty {
...
/// top-level shim in library/std/sync/src/lib.rx) and performs
