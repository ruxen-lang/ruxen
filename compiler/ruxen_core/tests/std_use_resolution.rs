...
use ruxen_core::{borrow_check, codegen, typeck};
use std::io::Write;
...
fn typecheck_source(source: &str) -> Vec<ruxen_core::diagnostics::Diagnostic> {
...
fn use_std_io_typechecks_cleanly() {
    let source = rx("use_std_io_typechecks_cleanly");
...
fn std_sync_concurrency_surface_typechecks_cleanly() {
    let source = rx("std_sync_concurrency_surface_typechecks_cleanly");
    let diagnostics = typecheck_source(&source);
    let errors: Vec<_> = diagnostics
        .iter()
        .filter(|d| d.level == DiagnosticLevel::Error)
...
fn std_sync_thread_sleep_and_yield_round_trip() {
    let source = rx("std_sync_thread_sleep_and_yield_round_trip");
...
fn std_io_group_imports_and_methods_typecheck_cleanly() {
    let source = rx("std_io_group_imports_and_methods_typecheck_cleanly");
    let diagnostics = typecheck_source(&source);
    let errors: Vec<_> = diagnostics
        .iter()
        .filter(|d| d.level == DiagnosticLevel::Error)
...
fn std_io_println_and_eprintln_round_trip() {
    let source = rx("std_io_println_and_eprintln_round_trip");
    let exe = compile_to_exe(&source);
    let output = Command::new(&exe)
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
...
fn std_io_write_str_result_is_unit_and_round_trips() {
    let source = rx("std_io_write_str_result_is_unit_and_round_trips");
    let exe = compile_to_exe(&source);
    let output = Command::new(&exe)
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
...
fn std_io_read_line_and_stdout_round_trip() {
    let source = rx("std_io_read_line_and_stdout_round_trip");
    let exe = compile_to_exe(&source);
...
fn std_io_stdin_read_to_string_round_trip() {
    let source = rx("std_io_stdin_read_to_string_round_trip");
    let exe = compile_to_exe(&source);
    let mut child = Command::new(&exe)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
...
fn main_shim_initializes_runtime_argv() {
    let source = rx("main_shim_initializes_runtime_argv");
    let exe = compile_to_exe(&source);
    let output = Command::new(&exe)
        .arg("alpha")
        .arg("beta")
...
fn use_std_env_typechecks_cleanly() {
    let source = rx("use_std_env_typechecks_cleanly");
    let diagnostics = typecheck_source(&source);
    let errors: Vec<_> = diagnostics
        .iter()
        .filter(|d| d.level == DiagnosticLevel::Error)
...
fn std_env_args_round_trip() {
    let source = rx("std_env_args_round_trip");
    let exe = compile_to_exe(&source);
    let output = Command::new(&exe)
        .arg("alpha")
        .arg("beta")
...
fn std_env_var_round_trip() {
    let source = rx("std_env_var_round_trip");
    let exe = compile_to_exe(&source);
    let output = Command::new(&exe)
        .env("RUXEN_STD_ENV_TEST", "env value")
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .output()
        .expect("run");
...
fn use_std_fs_typechecks_cleanly() {
    let source = rx("use_std_fs_typechecks_cleanly");
    let diagnostics = typecheck_source(&source);
    let errors: Vec<_> = diagnostics
        .iter()
        .filter(|d| d.level == DiagnosticLevel::Error)
...
fn std_fs_round_trip() {
    ...
    let id = COUNTER.fetch_add(1, Ordering::Relaxed);
    let path = std::env::temp_dir().join(format!("ruxen_std_fs_{}_{}.txt", std::process::id(), id));
    let path_str = path.to_string_lossy().replace('\\', "\\\\");
    let _ = std::fs::remove_file(&path);

    let source = rx("std_fs_round_trip").replace("{path}", &path_str);
...
fn use_std_process_typechecks_cleanly() {
    let source = rx("use_std_process_typechecks_cleanly");
    let diagnostics = typecheck_source(&source);
    let errors: Vec<_> = diagnostics
        .iter()
        .filter(|d| d.level == DiagnosticLevel::Error)
...
fn std_process_exit_round_trip() {
    let source = rx("std_process_exit_round_trip");
    let exe = compile_to_exe(&source);
    let output = Command::new(&exe)
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
...
fn use_std_fs_mutation_helpers_typecheck_cleanly() {
    let source = rx("use_std_fs_mutation_helpers_typecheck_cleanly");
    let diagnostics = typecheck_source(&source);
    let errors: Vec<_> = diagnostics
        .iter()
        .filter(|d| d.level == DiagnosticLevel::Error)
...
fn std_fs_mutation_helpers_round_trip() {
    ...
    let source = rx("std_fs_mutation_helpers_round_trip")
...
fn use_std_fs_create_dir_all_typechecks_cleanly() {
    let source = rx("use_std_fs_create_dir_all_typechecks_cleanly");
    let diagnostics = typecheck_source(&source);
    let errors: Vec<_> = diagnostics
        .iter()
        .filter(|d| d.level == DiagnosticLevel::Error)
...
fn std_fs_create_dir_all_round_trip() {
    ...
    let id = COUNTER.fetch_add(1, Ordering::Relaxed);
    let root = std::env::temp_dir().join(format!("ruxen_std_fs_all_{}_{}", std::process::id(), id));
    ...
    let source = rx("std_fs_create_dir_all_round_trip").replace("{nested}", &nested_str);
