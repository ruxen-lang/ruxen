...
//! stdlib file — `library/std/bootstrap_smoke/src/lib.rx` — that
//! declares `bootstrap_smoke_add_one` aliased to the runtime test
//! symbol `ruxen_test_extern_add_one`.
//!
...
use ruxen_core::resolve::bootstrap::{run_bootstrap_with_files, BOOTSTRAP_FILES};
...
/// We don't reach for `tempfile` to keep `ruxen_core`'s dev-deps lean.
struct TempDir {
    path: PathBuf,
}
...
impl TempDir {
    fn new() -> Self {
        ...
        base.push(format!("ruxen-bootstrap-prelude-{pid}-{nanos}-{n}"));
...
fn bootstrap_program_injects_def_into_user_scope() {
    ...
    assert_eq!(foo.ruxen_name, "foo");
    assert_eq!(foo.c_symbol.as_deref(), Some("ruxen_test_extern_add_one"));
    ...
    let mut found_ruxen_named = false;
    ...
    assert!(
        !found_ruxen_named,
        "Ruxen-side `foo` should not appear in MIR call sites — the alias rewrite is the whole point"
    );
}
...
fn bootstrap_smoke_e2e_via_runtime_file() {
    ...
    // `library/std/bootstrap_smoke/src/lib.rx`, merge into resolver,
    // compile a user .rx that calls `bootstrap_smoke_add_one`, link
...
fn bootstrap_class_method_e2e_via_runtime_file() {
    ...
    // `.rx` file with class-method FFI bindings is callable from user
    ...
    let bin_path = workspace_root().join(format!(
        "tmp/bootstrap_class_method_e2e-{}-{}.bin",
        std::process::id(),
        ruxen_unique_id()
    ));
    let _ = std::fs::create_dir_all(bin_path.parent().unwrap());
...
fn bootstrap_failure_aborts_driver() {
    ...
    // policy is documented in `ruxenc::main::load_bootstrap_or_exit`.
    let tmp = TempDir::new();
    write_fixture(
        tmp.path(),
        "broken.rx",
    ...
    let mut diags = Vec::<Diagnostic>::new();
    let programs = run_bootstrap_with_files(&["broken.rx"], Some(tmp.path()), &mut diags);
    ...
    assert!(
        diags
            .iter()
            .any(|d| d.message.contains("library/std/broken.rx")),
