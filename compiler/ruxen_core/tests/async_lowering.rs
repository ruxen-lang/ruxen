...
fn async_def_signature_lifts_to_future_or_state_machine() {
    let result = typeck_result(&rx("async_lowering_no_args"));
...
fn async_def_args_become_state_machine_fields() {
    let result = typeck_result(&rx("async_lowering_with_args"));
    let errors = error_messages(&result.diagnostics);
    assert!(errors.is_empty(), "fixture must typecheck: {:?}", errors);
...
fn poll_after_ready_returns_pending() {
    ...
    use ruxen_core::hir::nodes::HirExprKind;
...
fn await_desugars_to_poll_match_pending_return() {
    let result = typeck_result(&rx("async_lowering_single_await"));
    let errors = error_messages(&result.diagnostics);
    assert!(
        errors.is_empty(),
...
fn local_live_across_await_promoted_to_field() {
    let result = typeck_result(&rx("async_lowering_chained_await"));
    let errors = error_messages(&result.diagnostics);
    assert!(
        errors.is_empty(),
...
/// Spec B6: `Context.test_dummy` returns a Context the hand-druxen
/// poll loop can pass into `(&var fut).poll(&var ctx)`. The C
/// runtime side wires `ruxen_context_test_dummy` — see
...
fn context_test_dummy_constructs() {
    ...
    let ctx = result
        ...
        .expect("expected `Context` class from library/std/future/src/lib.rx");
...
///
/// Pin shape: see `tests/fixtures/ruxen/async_lowering_class_static_await.rx`
...
fn class_static_method_call_await_lowers() {
    let result = typeck_result(&rx("async_lowering_class_static_await"));
    let errors = error_messages(&result.diagnostics);
    assert!(
        errors.is_empty(),
...
fn pre_await_statement_lowers_to_segment_0_prefix() {
    let result = typeck_result(&rx("segmenter_pre_await_statement"));
    let errors = error_messages(&result.diagnostics);
    assert!(
        errors.is_empty(),
...
fn awaitee_arg_referencing_pre_await_local_captures_to_state() {
    let result = typeck_result(&rx("segmenter_awaitee_pre_await_local"));
    let errors = error_messages(&result.diagnostics);
    assert!(
        errors.is_empty(),
...
fn tail_referencing_pre_await_local_works() {
    let result = typeck_result(&rx("segmenter_tail_pre_await_local"));
    let errors = error_messages(&result.diagnostics);
    assert!(
        errors.is_empty(),
