//! Q2 regression — a method with an `Option[any Fn[...]]` field that
//! returns the result of an indirect closure call (`f.()`).
//!
//! The implicit fallthrough block synthesised after the `if let` (whose
//! arm `return`s) emitted a valueless `return` in a value-returning
//! function, tripping the Cranelift verifier:
//!   "arguments of return must match function signature".
//!
//! Compiles a real `.rx` program through lexer → parser → typeck → MIR →
//! Cranelift codegen and runs the produced binary.
...
#[test]
fn q2_option_fn_field_return() {
    let source = rx("q2_option_fn_field_return");
    let (stdout, stderr, ok) = compile_and_run(&source, "q2_option_fn_field_return");
