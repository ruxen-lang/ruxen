...
fn time_unix_ns_is_post_2020() {
    let source = rx("time_unix_ns_is_post_2020");
...
//
// Each test compiles a small `.rx` fixture that exercises one slice of
...
fn duration_from_secs_as_millis() {
    let source = rx("duration_from_secs_as_millis");
...
fn duration_from_millis_as_secs_floors() {
    let source = rx("duration_from_millis_as_secs");
...
fn duration_from_as_round_trip_matrix() {
    let source = rx("duration_round_trip_matrix");
...
fn duration_sub_saturates_to_zero() {
    let source = rx("duration_saturating_sub");
...
fn instant_monotonic_after_sleep() {
    let source = rx("instant_monotonic");
...
fn instant_duration_since_returns_delta() {
    let source = rx("instant_duration_since");
...
fn instant_duration_since_future_panics() {
    let source = rx("instant_duration_since_future_panics");
    let (stdout, stderr, ok) = compile_and_run(&source, "stdlib_time_inst_panic");
    assert!(
        !ok,
        "expected panic-druxen non-zero exit, but binary succeeded. \
...
fn sleep_duration_elapses_in_tolerance_band() {
    let source = rx("sleep_duration_elapses");
