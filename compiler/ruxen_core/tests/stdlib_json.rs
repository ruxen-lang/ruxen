...
fn compile_objects(extra_flags: &[&str]) -> Vec<PathBuf> {
    ...
    for src in json_runtime_sources(&root) {
        ...
        let obj = std::env::temp_dir().join(format!("ruxen_json_{stem}_{}.o", unique_suffix()));
...
fn compile_harness(name: &str, source: &str) -> PathBuf {
    ...
    let harness_c = std::env::temp_dir().join(format!("ruxen_json_{name}_{suffix}.c"));
    let harness_bin = std::env::temp_dir().join(format!("ruxen_json_{name}_{suffix}"));
...
int main(void) {
    ...
    void *result = ruxen_json_parse(src);
    if (tag(result) != 0) return 1;
    void *root = (void *)payload(result);

    void *answer_opt = ruxen_json_object_get(root, "answer");
    if (tag(answer_opt) != 1) return 2;
    void *answer_json = (void *)payload(answer_opt);
    void *answer_int = ruxen_json_as_int(answer_json);
    if (tag(answer_int) != 1 || payload(answer_int) != 42) return 3;

    void *flags_opt = ruxen_json_object_get(root, "flags");
    if (tag(flags_opt) != 1) return 4;
    void *flags_len = ruxen_json_array_len((void *)payload(flags_opt));
    if (tag(flags_len) != 1 || payload(flags_len) != 1) return 5;

    if (tag(ruxen_json_parse_strict(src)) != 1) return 6;
    return 0;
}
"#,
    );
...
int main(void) {
    if (tag(ruxen_json_parse_strict("// comment\n{\"a\": 1}")) != 1) return 1;
    if (tag(ruxen_json_parse_strict("{\"a\": 1,}")) != 1) return 2;
    if (tag(ruxen_json_parse_strict("[1, 2,]")) != 1) return 3;
    if (tag(ruxen_json_parse_strict("{\"a\": [1, 2]}")) != 0) return 4;
    return 0;
}
"#,
    );
...
typedef struct RuxenVec RuxenVec;
...
int main(void) {
    ...
    ruxen_vec_push(items, (int64_t)ruxen_json_make_int(7));
    ruxen_vec_push(items, (int64_t)ruxen_json_make_bool(1));

    RuxenHash *fields = ruxen_hash_new();
    ruxen_hash_insert(fields, (int64_t)ruxen_string_from("name"), (int64_t)ruxen_json_make_string("ruxen"));
    ruxen_hash_insert(fields, (int64_t)ruxen_string_from("items"), (int64_t)ruxen_json_make_array(items));
    ruxen_hash_insert(fields, (int64_t)ruxen_string_from("enabled"), (int64_t)ruxen_json_make_bool(1));

    void *root = ruxen_json_make_object(fields);
    void *enabled_opt = ruxen_json_object_get(root, "enabled");
    if (tag(enabled_opt) != 1) return 1;
    void *enabled_bool = ruxen_json_as_bool((void *)payload(enabled_opt));
    if (tag(enabled_bool) != 1 || payload(enabled_bool) != 1) return 2;

    void *name_opt = ruxen_json_object_get(root, "name");
    if (tag(name_opt) != 1) return 3;
    void *name_string = ruxen_json_as_string((void *)payload(name_opt));
    if (tag(name_string) != 1 || strcmp((const char *)payload(name_string), "ruxen") != 0) return 4;

    void *rendered = ruxen_json_stringify(root);
    if (tag(rendered) != 0) return 5;
    const char *json = (const char *)payload(rendered);
    if (!strstr(json, "\"name\":\"ruxen\"")) return 6;
