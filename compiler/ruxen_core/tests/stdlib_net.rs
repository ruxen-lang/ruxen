...
//! druxen from the host Rust thread (using `std::net::TcpListener` or
//! `std::net::TcpStream`), the other from the compiled Ruxen binary.
//! Where the host plays the listener role the chosen port is passed to
//! the child via `RUXEN_NET_TEST_PORT`; where Ruxen plays the listener
//! the test uses a host-side probe-bind to pick an ephemeral port the
//! Ruxen binary then re-binds.
...
/// surface. The Rust side binds an ephemeral port; the Ruxen binary
...
fn tcp_class_loopback_roundtrip() {
    ...
    let source = rx("tcp_class_loopback_roundtrip");
    let bin_path = compile(&source, "stdlib_net_class_roundtrip");

    let mut child = Command::new(&bin_path)
        .env("RUXEN_NET_TEST_PORT", port.to_string())
        .spawn()
    ...
    assert!(status.success(), "ruxen binary exited non-zero");
...
/// coordination: the Rust side accepts; the Ruxen binary connects and
...
fn tcp_stream_class_peer_addr() {
    ...
    let source = rx("tcp_stream_class_peer_addr");
...
/// peer's `read` to observe EOF. The Ruxen binary connects, calls
...
fn tcp_stream_class_shutdown_write_then_read_eof() {
    ...
    let source = rx("tcp_stream_class_shutdown_write_then_read_eof");
...
fn tcp_stream_class_set_read_timeout_would_block() {
    ...
    let source = rx("tcp_stream_class_set_read_timeout_would_block");
...
fn tcp_stream_class_set_write_timeout_resolves() {
    ...
    let source = rx("tcp_stream_class_set_write_timeout_resolves");
...
/// The host sends `[0xFF, 0x00, 0x41, 0x00, 0x42]`; the Ruxen child
/// connects, reads, and echoes the byte count + each byte back via
/// stdout. We assert the count is 5 (not 1, which would be the
/// truncate-at-first-NUL bug the deprecated `ruxen_tcp_read` would
/// have hit).
#[test]
fn tcp_stream_class_read_is_binary_safe() {
    ...
    let source = rx("tcp_stream_class_read_is_binary_safe");
...
/// each connection; the Ruxen binary connects 200 times in a loop and
...
fn tcp_stream_class_drop_closes_fd() {
    ...
    let source = rx("tcp_stream_class_drop_closes_fd");
...
fn tcp_class_prelude_auto_import_resolves() {
    let source = rx("tcp_class_prelude_auto_import_resolves");
    let mut lexer = Lexer::new(&source);
    let tokens = lexer.tokenize().expect("lex");
    let mut parser = Parser::new(tokens);
    let program = parser.parse().expect("parse");
...
/// C16 — the flat tcp_* free fns are no longer reachable from Ruxen
/// user code; importing them must produce a resolve-time error.
#[test]
fn flat_tcp_free_fns_removed_from_resolver() {
    let source = rx("flat_tcp_free_fns_removed_from_resolver");
...
/// End-to-end proof that Ruxen can host a blocking TCP server using
/// the new class surface.
///
/// The Ruxen program plays the **server** role using TcpListener +
/// TcpStream classes. It binds an ephemeral port (chosen by Rust +
/// handed off through `RUXEN_NET_PORT`), installs a SIGINT handler,
...
fn blocking_tcp_echo_server_with_graceful_sigint_shutdown() {
    ...
    let source = rx("blocking_tcp_echo_server_with_graceful_sigint_shutdown");
    let bin_path = compile(&source, "stdlib_net_server_sigint");

    let mut child = Command::new(&bin_path)
        .env("RUXEN_NET_PORT", port.to_string())
        .stdout(std::process::Stdio::piped())
