...
fn block_on_runs_subphase2a_future_to_ready() {
    let result = typeck_result(&rx("async_executor_block_on_no_await"));
...
/// Spec B8: chained-await futures druxen by block_on typecheck.
#[test]
fn block_on_runs_subphase2b_chained_future_to_ready() {
    let result = typeck_result(&rx("async_executor_block_on_with_chain"));
    let errors = error_messages(&result.diagnostics);
    assert!(
        errors.is_empty(),
...
fn block_on_preserves_typed_result_output() {
    let result = typeck_result(&rx("async_executor_block_on_typed_result"));
    let errors = error_messages(&result.diagnostics);
    assert!(
        errors.is_empty(),
...
/// in library/std/future/src/lib.rx are still wired to the same C
...
/// The inline poll-loop expression rewriting produces standard Ruxen
...
/// no Drop impl yet in Ruxen), the future state machine (heap, has
