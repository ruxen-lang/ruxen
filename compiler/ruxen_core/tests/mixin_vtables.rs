...
use ruxen_core::parser::ast::{DispatchMode, MixinDef, TopLevelItem};
...
fn parse(source: &str) -> ruxen_core::parser::ast::Program {
...
fn only_mixin(program: &ruxen_core::parser::ast::Program) -> &MixinDef {
...
fn mixin_dispatch_runtime_modifier_parses() {
    let source = rx("mixin_dispatch_runtime_modifier_parses");
...
fn mixin_without_dispatch_runtime_defaults_to_static() {
    ...
    // `mixin_and_include.rx` which predates this feature.
    let source = rx("mixin_and_include");
    let program = parse(&source);
    let mixin = only_mixin(&program);
    assert_eq!(
        mixin.dispatch_mode,
...
fn dyn_mixin_param_type_parses_and_typechecks() {
    let source = rx("dyn_mixin_param_type_parses_and_typechecks");
...
fn amp_mixin_to_static_mixin_emits_e1118() {
    let source = rx("dyn_mixin_ref_to_static_mixin_emits_e1118");
    let errs = errors(&source);
    let count = errs
        .iter()
...
fn runtime_dispatch_mixin_missing_method_emits_e1117() {
    let source = rx("runtime_mixin_missing_method_emits_e1117");
    let errs = errors(&source);
    let count = errs
        .iter()
...
fn class_includes_runtime_mixin_is_tracked_on_classinfo() {
    ...
    let sized_def_id = result
        ...
        .find(|d| {
            d.name == "Sized"
                && matches!(
                    &d.kind,
                    ruxen_core::resolve::symbols::DefKind::Trait { info }
    ...
    let circle_info = result
        ...
        .map(|d| match &d.kind {
            ruxen_core::resolve::symbols::DefKind::Class { info } => info.clone(),
...
fn lower_fixture(name: &str) -> ruxen_core::mir::nodes::MirProgram {
    ...
    let mut lowerer = ruxen_core::mir::lower::Lowerer::new(&result.symbols);
    lowerer
...
fn class_info_struct_emitted_per_runtime_dispatch_class() {
    ...
    assert_eq!(
        ci.vtable_symbols,
        vec!["__rx_vtable_Sized_for_Circle".to_string()],
        "class_info should point at the (Circle, Sized) vtable",
    );
    assert_eq!(
        ci.symbol(),
        "__rx_classinfo_Circle",
...
fn runtime_dispatch_class_field_index_shifted_by_one() {
    ...
    let radius_set_slots: Vec<usize> = init_fn
        ...
        .filter_map(|i| match i {
            ruxen_core::mir::nodes::MirInst::SetField { field_index, .. } => Some(*field_index),
            _ => None,
        })
        .collect();
    ...
    let radius_get_slots: Vec<usize> = size_fn
        ...
        .flat_map(|b| b.instructions.iter())
        .filter_map(|i| match i {
            ruxen_core::mir::nodes::MirInst::GetField { field_index, .. } => Some(*field_index),
            _ => None,
        })
        .collect();
    assert!(
...
fn static_mixin_class_field_index_unshifted() {
    ...
    // is emitted at slot 0 in any Bob method (no `__rx_classinfo_Bob`
    // exists, so emitting one would be a hard codegen error).
    let bob_methods: Vec<&ruxen_core::mir::nodes::MirFunction> = mir
        .functions
        .iter()
    ...
    for f in &bob_methods {
        let dataaddrs: Vec<&str> = f
            ...
            .flat_map(|b| b.instructions.iter())
            .filter_map(|i| match i {
                ruxen_core::mir::nodes::MirInst::DataAddr { data_sym, .. } => {
...
fn async_state_machine_init_auto_assigns_use_header_shift() {
    ...
    for inst in &entry.instructions {
        match inst {
            ruxen_core::mir::nodes::MirInst::SetField { field_index, .. } => {
...
/// `DataAddr { data_sym: "__rx_classinfo_<Class>" }` followed by a
...
fn class_init_writes_class_info_ptr_at_slot_zero() {
    ...
    for block in &make_one.blocks {
        let mut last_data_addr: Option<(ruxen_core::mir::nodes::LocalId, String)> = None;
        for inst in &block.instructions {
            match inst {
                ruxen_core::mir::nodes::MirInst::DataAddr { dest, data_sym } => {
                    last_data_addr = Some((*dest, data_sym.clone()));
                }
                ruxen_core::mir::nodes::MirInst::SetField {
                    field_index: 0,
                    value: ruxen_core::mir::nodes::MirValue::Use(v_local),
                    ..
                } => {
                    if let Some((da_dest, da_sym)) = &last_data_addr {
                        if *v_local == *da_dest && da_sym == "__rx_classinfo_Circle" {
                            found = true;
                        }
                    }
                }
    ...
    assert!(
        found,
        "make_one must emit `DataAddr __rx_classinfo_Circle` → `SetField slot 0` pair \
...
fn callees_in(f: &ruxen_core::mir::nodes::MirFunction) -> Vec<String> {
    f.blocks
        .iter()
        .flat_map(|b| b.instructions.iter())
        .filter_map(|i| match i {
            ruxen_core::mir::nodes::MirInst::Call { callee, .. } => Some(callee.clone()),
            _ => None,
        })
        .collect()
}
...
fn dynamic_dispatch_helper_synthesized_per_mixin_method() {
    ...
    for block in &helper.blocks {
        for inst in &block.instructions {
            match inst {
                ruxen_core::mir::nodes::MirInst::GetField { field_index, .. } => {
                    get_field_slots.push(*field_index);
                }
                ruxen_core::mir::nodes::MirInst::CallIndirect { .. } => {
