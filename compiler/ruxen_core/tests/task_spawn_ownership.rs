...
//! block. Ruxen's drop elaboration must recognise this move so the
//! caller's local is NOT freed at scope exit — otherwise a
//! short-lived spawner fn returns, its `fut` local is drop-
//! elaborated, and the next `ruxen_executor_pump_tasks()` call in
//! the outer driver dereferences freed memory (SIGSEGV / exit 139).
//!
//! The fix lives in `compiler/ruxen_core/src/mir/lower/drops.rs`:
//! the `is_move_by_ffi_callee` predicate excludes
//! `ruxen_executor_spawn` from the runtime-borrow-helper list, so
//! the default-taint path on the per-arg loop runs and the
//! spawned future's local is removed from `alloc_rooted`.
//!
//! Discipline: all Ruxen source lives in the
//! `tests/fixtures/ruxen/` tree, never inline in this `.rs` file
//! (see `feedback_no_inline_rx_in_pin_tests`).
...
/// exit; the next `ruxen_executor_pump_tasks()` from `main`
/// followed a freed pointer and SIGSEGV'd (exit 139).
///
/// After the fix: `ruxen_executor_spawn` is excluded from the
...
fn task_spawn_raw_transfers_ownership_to_executor() {
    let source = rx("task_spawn_move_by_ffi");
...
fn task_spawn_raw_sync_scope_transfers_ownership_to_executor() {
    let source = rx("task_spawn_raw_sync_scope_transfer");
...
fn task_waker_selective_ready_queue_polls_only_woken_task() {
    let source = rx("task_waker_selective_ready_queue");
