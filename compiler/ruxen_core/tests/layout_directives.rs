...
//!
//! Ruxen source goes through `tests/fixtures/ruxen/layout_*.rx` —
...
fn resolve_fixture(name: &str) -> ruxen_core::resolve::ResolveResult {
...
fn class_with_layout_c_parses_and_resolves() {
    ...
    let errors: Vec<_> = result
        .diagnostics
        .iter()
        .filter(|d| matches!(d.level, ruxen_core::diagnostics::DiagnosticLevel::Error))
        .collect();
    assert!(
        errors.is_empty(),
