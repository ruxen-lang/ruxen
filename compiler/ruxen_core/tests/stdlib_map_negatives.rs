...
//! see `crates/ruxen-core/src/resolve/mod.rs::ty_is_valid_hash_key`.
//!
...
//! (`tests/release-e2e/cases/50[1-9]_hashmap_*.rx`,
//! `52[1-7]_hashset_*.rx`).
...
fn hashmap_with_non_hash_key_emits_e0615() {
    let source = rx("hashmap_with_non_hash_key_emits_e0615");
...
fn hashset_with_non_hash_element_emits_e0615() {
    let source = rx("hashset_with_non_hash_element_emits_e0615");
    let diags = typecheck_diagnostics_from_source(&source);
    let errs: Vec<_> = diags
        .iter()
        .filter(|d| d.level == DiagnosticLevel::Error)
...
fn hashmap_with_nested_compound_key_emits_e0615() {
    let source = rx("hashmap_with_nested_compound_key_emits_e0615");
    let diags = typecheck_diagnostics_from_source(&source);
    let cs = codes(&diags);
    assert!(
        cs.iter().any(|c| c == "E0615"),
...
fn hashset_of_hashmap_emits_e0615() {
    let source = rx("hashset_of_hashmap_emits_e0615");
    let diags = typecheck_diagnostics_from_source(&source);
    let cs = codes(&diags);
    assert!(
        cs.iter().any(|c| c == "E0615"),
...
fn hashmap_with_vec_value_is_accepted() {
    let source = rx("hashmap_with_vec_value_is_accepted");
    let diags = typecheck_diagnostics_from_source(&source);
    let errs: Vec<_> = diags
        .iter()
        .filter(|d| d.level == DiagnosticLevel::Error)
...
fn hashmap_with_string_key_is_accepted() {
    let source = rx("hashmap_with_string_key_is_accepted");
    let diags = typecheck_diagnostics_from_source(&source);
    let errs: Vec<_> = diags
        .iter()
        .filter(|d| d.level == DiagnosticLevel::Error)
...
fn hashmap_entry_then_or_insert_split_is_rejected() {
    let source = rx("hashmap_entry_then_or_insert_split_is_rejected");
    let diags = typecheck_diagnostics_from_source(&source);
    let errs: Vec<_> = diags
        .iter()
        .filter(|d| d.level == DiagnosticLevel::Error)
...
fn hashmap_or_insert_on_non_entry_receiver_rejected() {
    let source = rx("hashmap_or_insert_on_non_entry_receiver_rejected");
    let diags = typecheck_diagnostics_from_source(&source);
    let errs: Vec<_> = diags
        .iter()
        .filter(|d| d.level == DiagnosticLevel::Error)
