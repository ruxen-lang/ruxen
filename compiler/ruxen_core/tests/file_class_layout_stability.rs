...
//!
//! 1. `RuxenFile` is exactly 8 bytes — `{ int32 fd; int32 closed }`.
//!    The MIR scope-exit drop pipeline emits `File_drop(f) +
//!    ruxen_dealloc(f)` against an 8-byte allocation; widening the
//!    struct without updating that contract would either leak the
//!    extra bytes or, worse, alias into the next heap block.
//!
//! 2. `RuxenOpenOptions` is exactly 8 bytes — six u8 flag bytes plus
...
//!    `ruxen_open_options_new` allocation site, every Cranelift/LLVM
//!    extern decl, and the runtime accessors to move in lockstep.
//!
//! 3. `SeekFrom` tag values match the runtime's `RUXEN_SEEK_FROM_*`
//!    constants 1:1. The resolver-side enum is `Start(Int) / End(Int)
//!    / Current(Int)` — three single-field struct variants whose
//!    16-byte tagged-value layout is read by `ruxen_file_seek`.
//!
...
// library/std/io/src/lib.rx; the pin test now scans .rx declaration
// order directly, so the tuple parser is no longer needed.

#[test]
fn ruxen_file_static_assert_is_eight_bytes() {
    ...
    assert!(
        runtime.contains("_Static_assert(sizeof(RuxenFile) == 8"),
        "expected `_Static_assert(sizeof(RuxenFile) == 8 ...)` in runtime.c — \
...
#[test]
fn ruxen_open_options_static_assert_is_eight_bytes() {
    let runtime = read("library/std/io/runtime/file.c");
    assert!(
        runtime.contains("_Static_assert(sizeof(RuxenOpenOptions) == 8"),
        "expected `_Static_assert(sizeof(RuxenOpenOptions) == 8 ...)` in runtime.c"
    );
}
...
fn seek_from_tag_values_match_runtime_and_stdlib_source() {
    ...
    // `compiler/ruxen_core/src/resolve/stdlib/mod.rs` into
    // `library/std/io/src/lib.rx`. The resolver-side scan target moved
    // with it; the runtime side (`#define RUXEN_SEEK_FROM_*` in
    // library/std/io/runtime/file.c) is unchanged.
    let runtime = read("library/std/io/runtime/file.c");
    let stdlib_source = read("library/std/io/src/lib.rx");

    // Runtime side: scan for `#define RUXEN_SEEK_FROM_<NAME>  <tag>`.
    let mut runtime_tags: Vec<(String, usize)> = Vec::new();
    for line in runtime.lines() {
        let trimmed = line.trim();
        let prefix = "#define RUXEN_SEEK_FROM_";
