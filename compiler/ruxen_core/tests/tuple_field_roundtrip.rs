...
use ruxen_core::parser::ast::ExprKind;
...
fn tuple_field_multi_digit_index_preserves_value() {
    ...
    let main = prog
        ...
        .iter()
        .find_map(|item| {
            if let ruxen_core::parser::ast::TopLevelItem::Function(f) = item {
                if f.name == "main" {
                    Some(f)
                } else {
                    None
    ...
    let value = match stmt {
        ruxen_core::parser::ast::Statement::Let(l) => l.value.as_ref().expect("value"),
