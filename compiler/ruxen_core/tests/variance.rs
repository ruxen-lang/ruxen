...
//! constructor is allowed. The Ruxen compiler currently encodes these
//! rules in [`ruxen_core::typeck::coerce::try_coerce`] and the matching
//! [`ruxen_core::typeck::unify::can_coerce`] predicate, but as of the
...
//!
//! The Ruxen surface syntax does not (yet) expose explicit subtype
//! lifetimes or other constructions that would let us write a plain
//! `.rx` source whose ill-typedness depends *only* on a variance rule.
//! In particular:
//!
...
//! fixtures `tests/release-e2e/cases/136_variance_option_covariant.rx`
//! and `137_variance_array_same_type.rx`.

use ruxen_core::hir::context::TypeContext;
use ruxen_core::hir::types::Ty;
use ruxen_core::typeck::unify::can_coerce;
...
fn vec_same_type_works() {
    ...
    // `137_variance_array_same_type.rx` covers the same case at the
