...
//! `library/std/io/src/lib.rx` can write:
//!
//! ```ruxen
//! class File
...
//!
//!   lib "ruxen_runtime"
//!     def open as "ruxen_file_open"(path: &String) -> Result[File, IoError]
//!   end
//! end
//! ```
//!
...
//!
//! Test discipline: no inline `r#"..."#` Ruxen source. All Ruxen goes
//! through `.rx` fixtures under `tests/fixtures/ruxen/` (project rule
//! `feedback_no_inline_rx_in_pin_tests`).

use ruxen_core::lexer::Lexer;
use ruxen_core::parser::ast::{ClassDef, MixinDef, TopLevelItem};
...
fn only_class(program: &ruxen_core::parser::ast::Program) -> &ClassDef {
...
fn lib_block_in_class_body_parses() {
    let program = parse_fixture("tests/fixtures/ruxen/lib_in_class_body_single.rx")
...
fn multiple_lib_blocks_in_class_body_parse() {
    let program = parse_fixture("tests/fixtures/ruxen/lib_in_class_body_multiple.rx")
        .expect("fixture should parse");
    let class = only_class(&program);
    assert_eq!(
        class.lib_decls.len(),
...
fn lib_block_in_mixin_body_parses() {
    let program =
        parse_fixture("tests/fixtures/ruxen/lib_in_mixin_body.rx").expect("fixture should parse");
