...
use ruxen_core::mir::lower::lower_program;
use ruxen_core::mir::nodes::{MirInst, MirProgram};
...
fn user_impl_display_lowers_t_fmt_function() {
    // Ruxen syntax: `include Display` declares the Display contract;
    // `def fmt` has no explicit `self` param — `self` is implicit.
    let src = rx("user_impl_display_lowers_t_fmt_function");
    let m = lower(&src);
    let names: Vec<&str> = m.functions.iter().map(|f| f.name.as_str()).collect();
    assert!(
...
// `crates/ruxen-core/src/mir/tests.rs::helper_tests::user_has_impl_display_resolves_class_int_and_ref`.
...
/// `Formatter_buffer` — and the legacy direct call to `ruxen_int_to_string`
/// at the interp site is gone.  The synth `Int_fmt` fn (Stage 1) still wraps
/// `ruxen_int_to_string` internally, so existing fixture output is preserved.
#[test]
fn interpolation_primitive_goes_through_display() {
    let src = rx("interpolation_primitive_goes_through_display");
    let m = lower(&src);
    let main = m
        .functions
        .iter()
    ...
    assert!(
        !callees.contains(&"ruxen_int_to_string"),
        "old direct call to ruxen_int_to_string should be gone from main; got: {:?}",
        callees
    );
}
