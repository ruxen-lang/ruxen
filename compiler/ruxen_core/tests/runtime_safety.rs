...
fn compile_runtime_objects(extra_flags: &[&str]) -> Vec<std::path::PathBuf> {
    ...
    for src in &sources {
        ...
        let obj = std::env::temp_dir().join(format!("ruxen_{}_{}.o", stem, unique_suffix()));
...
fn compile_c_harness(name: &str, source: &str) -> PathBuf {
    ...
    // `Future_dynamic_poll` — the symbol Ruxen codegen synthesises for
    ...
    // Inject a weak stub that's overridden when Ruxen *does* define
...
int main(void) {
    ...
    ruxen_env_init(2, argv);
    arg1[0] = 'X';

    if (ruxen_env_args_count() != 2) {
        return 1;
    }

    char *copy1 = ruxen_env_args_at(1);
    char *copy2 = ruxen_env_args_at(1);
    if (!copy1 || !copy2) {
        return 2;
    }
...
int main(int argc, char **argv) {
    ...
    setenv("RUXEN_RUNTIME_ENV_TEST", "expected-value", 1);

    void *env_ok = ruxen_env_var("RUXEN_RUNTIME_ENV_TEST");
    if (!is_ok(env_ok) || strcmp(payload_str(env_ok), "expected-value") != 0) {
        return 1;
    }

    void *env_err = ruxen_env_var("RUXEN_RUNTIME_ENV_MISSING");
    if (!is_err(env_err)) {
        return 2;
    }

    if (ruxen_fs_exists(file_a) != 0) {
        return 3;
    }
    if (!is_ok(ruxen_fs_write(file_a, "hello runtime"))) {
        return 4;
    }
    if (ruxen_fs_exists(file_a) != 1) {
        return 5;
    }

    void *read_back = ruxen_fs_read_to_string(file_a);
    if (!is_ok(read_back) || strcmp(payload_str(read_back), "hello runtime") != 0) {
        return 6;
    }

    if (!is_ok(ruxen_fs_rename(file_a, file_b))) {
        return 7;
    }
    if (ruxen_fs_exists(file_a) != 0 || ruxen_fs_exists(file_b) != 1) {
        return 8;
    }

    if (!is_ok(ruxen_fs_create_dir(dir_a))) {
        return 9;
    }
    if (!is_ok(ruxen_fs_remove_file(file_b))) {
        return 10;
    }
    if (ruxen_fs_exists(file_b) != 0) {
        return 11;
    }

    ruxen_process_exit(23);
    return argc;
}
"#,
    );
    ...
    let temp_root = std::env::temp_dir().join(format!(
        "ruxen_runtime_fs_env_process_{}_{}",
        std::process::id(),
        unique
    ));
...
mod proptest_tests {
    ...
    use ruxen_core::lexer::token::{lookup_keyword, TokenKind};
