...
//!    Codes already emitted by `crates/ruxen-core/src/implicit_includes/mod.rs`
...
use ruxen_core::diagnostics::codes::{is_registered, REGISTRY};

/// Pre-existing codes emitted by `crates/ruxen-core/src/implicit_includes/mod.rs`
...
fn e0601_through_e0609_emitted_codes_are_registered() {
    ...
    assert!(
        ...
        "the following codes are emitted by `implicit_includes/mod.rs` but absent \
         ...
         row for each in `crates/ruxen-core/src/diagnostics/codes.rs`.",
        missing
    );
}
