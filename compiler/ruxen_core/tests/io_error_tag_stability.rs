...
//! stdlib source (`library/std/io/src/lib.rx`) co-document the tag
...
fn io_error_variant_tags_match_runtime_and_stdlib_source() {
    // #06.95 Phase B-2: the RUXEN_IO_ERROR_* tag macros moved from
    ...
    for line in runtime.lines() {
        ...
        let prefix = "#define RUXEN_IO_ERROR_";
    ...
    // io.rx and assign each non-comment, non-blank line its
    ...
    assert_eq!(
        stdlib_tags.len(),
        20,
        "expected 20 IoError variants in library/std/io/src/lib.rx; got {:?}",
        stdlib_tags
    );
    assert_eq!(
