...
//!
//! Ruxen sources live under `tests/fixtures/ruxen/<test_name>.rx`
//! so future syntax migrations sweep `*.rx` uniformly.

use ruxen_core::lexer::Lexer;
use ruxen_core::parser::ast::{Program, TopLevelItem};
...
fn parse_module_def_basic() {
    let src = rx("parse_module_def_basic");
    let prog = parse(&src);
    let module = prog
        .items
        .iter()
...
fn parse_nested_modules() {
    let src = rx("parse_nested_modules");
    let prog = parse(&src);
    let outer = prog
        .items
        .iter()
...
fn parse_module_with_diverse_items() {
    let src = rx("parse_module_with_diverse_items");
    let prog = parse(&src);
    let module = prog
        .items
        .iter()
