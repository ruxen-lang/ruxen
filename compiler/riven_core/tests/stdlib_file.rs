...
//! Riven program that exercises one File surface, runs the binary,
//! and asserts on stdout. Path templating mirrors `stdlib_fs.rs` —
//! the `.rvn` fixtures contain `__FILE__` / `__DIR__` placeholders
...
fn file_open_read_to_string_round_trips() {
    ...
    let source = rvn("file_open_read_to_string_round_trips")
...
fn file_open_missing_returns_not_found() {
    ...
    let source = rvn("file_open_missing_returns_not_found")
...
fn file_append_preserves_existing_bytes() {
    ...
    let source = rvn("file_append_preserves_existing_bytes")
        .replace("__FILE__", &file.display().to_string());
...
fn file_open_options_no_access_mode_returns_invalid_input() {
    ...
    let source = rvn("file_open_options_no_access_mode_returns_invalid_input")
        .replace("__FILE__", &file.display().to_string());
