...
//!    mirror keeps these cases reachable from `cargo test -p riven_core`).
//!
//! Every Riven snippet lives in `tests/fixtures/riven/` — no inline
...
fn bufreader_inner_non_file_or_tcp_emits_e0714() {
    let source = rvn("bufreader_inner_string_emits_e0714");
...
// ── E2E mirror tests ──────────────────────────────────────────────────
//
// The release-e2e harness uses RIVEN_E2E_CASES to narrow runs; if that
// env-var is blocked the cases below are still reachable through
// `cargo test -p riven_core --test stdlib_bufio`. Each mirror compiles
