...
//! `tests/release-e2e/cases/600_closure_handler_dispatch.rvn` fixture
//! so the regression is caught by `cargo test -p riven_core` even
...
fn dyn_fn_let_binding_coerces() {
    let source = rvn("dyn_fn_let_binding_coerces");
...
fn dyn_fn_array_store_and_dispatch() {
    let source = rvn("dyn_fn_array_store_and_dispatch");
...
fn dyn_fn_router_pattern() {
    let source = rvn("dyn_fn_router_pattern");
...
fn dyn_fn_return_from_function() {
    let source = rvn("dyn_fn_return_from_function");
    let (stdout, stderr, ok) = compile_and_run(&source, "dyn_fn_return_from_function");
    assert!(ok, "non-zero exit; stderr: {}", stderr);
    assert_eq!(stdout, "7\n", "stdout was {:?}", stdout);
}

/// 5. Mirrors `tests/release-e2e/cases/600_closure_handler_dispatch.rvn`
