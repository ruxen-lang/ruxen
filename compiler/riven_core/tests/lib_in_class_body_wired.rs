...
//! register as class methods (the Riven equivalent of `def self.foo`)
...
fn class_lib_method_registered_as_class_method() {
    ...
    assert_eq!(lib.functions[0].riven_name, "Foo_bar");
    assert_eq!(
...
fn class_lib_method_mir_callee_is_c_symbol() {
    ...
    assert!(
        found_aliased,
        "MIR Call should use C symbol `riven_test_extern_add_one`; got {:?}",
        main_fn
    );
    assert!(
        !found_mangled,
