...
//! `==` lowers to `riven_string_eq` (strcmp) rather than integer
//! pointer-eq.
//!
...
//! (`library/std/string/src/lib.rvn`) onto the same DefId so the FFI
...
/// `riven_string_eq`. Before the fix, both operands were
...
/// against a String parameter must lower to `riven_string_eq`. This
...
fn string_eq_after_result_ok_destructure_uses_strcmp() {
    let (stdout, ok) = compile_and_run(
        &rvn("string_eq_after_result_destructure"),
