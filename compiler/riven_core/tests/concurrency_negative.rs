...
fn mutex_new_rejects_non_send_t_e1101() {
    let errs = typeck_errors(&rvn("concurrency_mutex_new_rejects_non_send"));
    assert!(
...
fn sharedsync_new_rejects_non_send_t_e1102() {
    let errs = typeck_errors(&rvn("concurrency_sharedsync_new_rejects_non_send"));
    assert!(
...
// library/std/sync/src/lib.rvn — Sender / Receiver are bare class
...
fn thread_spawn_rejects_non_send_capture_e1100() {
    let errs = typeck_errors(&rvn("concurrency_thread_spawn_rejects_non_send_capture"));
    assert!(
...
// The current Riven closure surface doesn't distinguish move vs ref
// captures of class values at the .rvn level (move semantics are the
...
fn thread_spawn_emits_one_diagnostic_per_bad_capture() {
    let errs = typeck_errors(&rvn("concurrency_thread_spawn_multiple_captures"));
...
fn negative_include_send_rejects_thread_spawn() {
    ...
    let errs = typeck_errors(&rvn("concurrency_negative_include_send_rejects"));
    assert!(
        count_with_code(&errs, "E1101") >= 1,
