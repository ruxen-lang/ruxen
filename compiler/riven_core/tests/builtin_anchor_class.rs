...
//!    mangled `String_<method>` riven_name and the C-symbol alias —
...
fn anchor_mode_routes_class_body_lib_decl_into_ffi_libs() {
    ...
    // The mangled riven_name is `String_len_via_anchor` (the parent name
    ...
    let anchored = string_lib
        .functions
        .iter()
        .find(|f| f.riven_name == "String_len_via_anchor")
        .unwrap_or_else(|| {
            panic!(
                "expected riven_name 'String_len_via_anchor' in String ffi lib; got fns = {:?}",
                string_lib
                    .functions
                    .iter()
                    .map(|f| &f.riven_name)
