...
//! `compiler/riven_core/src/` beyond a single `BOOTSTRAP_FILES`
...
//!    must touch nothing under `compiler/riven_core/src/` beyond
...
fn foobar_addition_touches_only_bootstrap_files() {
    ...
    for path in diff.lines() {
        if !path.starts_with("compiler/riven_core/src/") {
            continue;
        }
        if path == "compiler/riven_core/src/resolve/bootstrap.rs" {
    ...
    assert!(
        offenders.is_empty(),
        "adding library/std/foobar/ touched compiler/riven_core/src/ files \
