...
fn fs_is_file_distinguishes_regular_files() {
    ...
    let source = rvn("fs_is_file_distinguishes_regular_files")
...
fn fs_is_dir_distinguishes_directories() {
    ...
    let source = rvn("fs_is_dir_distinguishes_directories")
        .replace("__DIR__", &dir.display().to_string())
        .replace("__FILE__", &file.display().to_string())
...
fn fs_read_dir_lists_all_entries() {
    ...
    let source =
        rvn("fs_read_dir_lists_all_entries").replace("__DIR__", &dir.display().to_string());
...
fn fs_read_dir_missing_path_returns_err() {
    let source = rvn("fs_read_dir_missing_path_returns_err");
...
fn fs_write_then_read_to_string_round_trips() {
    ...
    let file = dir.join("a.txt");
    let source = rvn("fs_write_then_read_to_string_round_trips")
        .replace("__FILE__", &file.display().to_string());
...
fn fs_read_to_string_missing_returns_err() {
    let source = rvn("fs_read_to_string_missing_returns_err");
...
fn fs_exists_distinguishes_existing_and_missing() {
    ...
    let source = rvn("fs_exists_distinguishes_existing_and_missing")
        .replace("__FILE__", &file.display().to_string())
        .replace("__MISSING__", &dir.join("nope").display().to_string());
...
fn fs_create_dir_then_is_dir() {
    ...
    let source = rvn("fs_create_dir_then_is_dir").replace("__SUB__", &sub.display().to_string());
...
fn fs_remove_file_then_exists_false() {
    ...
    let source =
        rvn("fs_remove_file_then_exists_false").replace("__FILE__", &file.display().to_string());
