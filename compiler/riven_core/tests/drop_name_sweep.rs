//! B4 pin tests — no `def __drop` remains in stdlib `.rvn` files.
//!
//! `mir/lower/collect.rs::collect_user_drop_classes` matches the
//! literal method name `drop`. Any stdlib `.rvn` class declaring its
...
//! lib.rvn`; B4 of `docs/specs/system/zero_rust_stdlib_classes.spec.md`
//! sweeps them to `def drop`.
//!
//! This pin scans every `library/std/*/src/lib.rvn` for `def __drop`
...
fn walk_lib_rvns(root: &Path, out: &mut Vec<PathBuf>) {
    ...
    for entry in entries.flatten() {
        let path = entry.path();
        if path.is_dir() {
            // Look for `<pkg>/src/lib.rvn`.
            let candidate = path.join("src").join("lib.rvn");
...
fn no_double_underscore_drop_remains_in_stdlib() {
    ...
    walk_lib_rvns(&root, &mut lib_files);
    assert!(
        !lib_files.is_empty(),
        "no library/std/*/src/lib.rvn files found under {}",
        root.display(),
    );
    ...
    for file in &lib_files {
        ...
        for (idx, line) in contents.lines().enumerate() {
            // Skip comments — `#` and `##` are line-comments in Riven.
    ...
    assert!(
        offenders.is_empty(),
        "stdlib `.rvn` files must declare Drop hooks as `def drop as \"...\"`, never `def __drop` \
