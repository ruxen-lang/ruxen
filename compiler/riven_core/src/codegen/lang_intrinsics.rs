...
//!    `riven_iter_to_vec` with the migrated `Vec[T]_iter`. E0722
//!    rejects duplicate aliases for the same `c_symbol` with
//!    different wire shapes, so they cannot ride the
//!    `library/std/array/src/lib.rvn` alias map. Same story for
//!    `Vec[T]_get_mut` / `Vec[T]_get_var` aliasing
//!    `riven_vec_get_opt` with `get`. The `Option[T]_unwrap!` /
...
//! is driven by the MIR call-site lowerer in
...
pub fn runtime_name(name: &str) -> Result<&str, String> {
        ...
        || name.starts_with("SplitIter")
    {
        return match method {
            ...
            // the v1 runtime already hands back a `RivenVec *`,
            ...
            // fresh `RivenVec *` via the `riven_vec_take` /
            // `riven_vec_skip` helpers. Closure-taking
            ...
            // `RivenVec*`s via the runtime helpers below.
            // `collect_vec` is the v1 type-specific shorthand
            // for `collect[Vec[T]]` — since every `*Iter` is
            // already a `RivenVec*` at runtime, the collector
    ...
    // library/std/array/src/lib.rvn as a `class Array do lib
    ...
    //     `as_slice` → `riven_iter_to_vec` (canonical spelling
    //     `iter` is in array.rvn)
    if name.starts_with("Array") || name.starts_with("Vec") {
        return match method {
            ...
            // is already represented by a `RivenVec *`, so
    ...
    // `ok_or` into library/std/option_result/src/lib.rvn as a
    // `class Option do lib "riven_runtime" ... end end` shell.
    ...
    // `ok`, `err` into the same option_result.rvn lib block.
    ...
    // historical `_ => riven_noop_passthrough` fallback here
    ...
    if name.starts_with("?T") || name.starts_with('?') {
        return match method {
            ...
            // change — the old `_ => "riven_noop_passthrough"`
