 ...
 *
 * This is a strict, Riven-native JSON implementation. It borrows the
...
#define RIVEN_JSON_NULL   0
#define RIVEN_JSON_BOOL   1
#define RIVEN_JSON_INT    2
#define RIVEN_JSON_FLOAT  3
#define RIVEN_JSON_STRING 4
#define RIVEN_JSON_ARRAY  5
#define RIVEN_JSON_OBJECT 6

#define RIVEN_JSON_ERROR_SYNTAX              0
#define RIVEN_JSON_ERROR_DEPTH_LIMIT         1
#define RIVEN_JSON_ERROR_INVALID_UTF8        2
#define RIVEN_JSON_ERROR_NUMBER_OUT_OF_RANGE 3
...
static void *json_error(int tag, const char *message) {
    ...
    *(int32_t *)err = tag;
    err[1] = (tag == RIVEN_JSON_ERROR_SYNTAX)
        ? (int64_t)riven_string_from(message ? message : "invalid JSON")
        : 0;
    return err;
}
...
static void *json_null(void) {
    return json_value(RIVEN_JSON_NULL, 0);
}

static void *json_bool(int value) {
    return json_value(RIVEN_JSON_BOOL, value ? 1 : 0);
}

static void *json_int(int64_t value) {
    return json_value(RIVEN_JSON_INT, value);
}

static void *json_float(double value) {
    union { double d; int64_t i; } u;
    u.d = value;
    return json_value(RIVEN_JSON_FLOAT, u.i);
}

void *riven_json_make_null(void) {
    return json_null();
}
...
void *riven_json_make_float(double value) {
    return json_float(value);
}

void *riven_json_make_string(const char *value) {
    return json_value(RIVEN_JSON_STRING, (int64_t)riven_string_from(value ? value : ""));
}

void *riven_json_make_array(RivenVec *items) {
    return json_value(RIVEN_JSON_ARRAY, (int64_t)(items ? items : riven_vec_new()));
}

void *riven_json_make_object(RivenHash *fields) {
    RivenHash *obj = fields ? fields : riven_hash_new();
    obj->string_keys = 1;
    return json_value(RIVEN_JSON_OBJECT, (int64_t)obj);
}

void *riven_json_make_empty_array(void) {
    return json_value(RIVEN_JSON_ARRAY, (int64_t)riven_vec_new());
}

void *riven_json_make_empty_object(void) {
    RivenHash *obj = riven_hash_new();
    obj->string_keys = 1;
    return json_value(RIVEN_JSON_OBJECT, (int64_t)obj);
}
...
static void skip_ws(JsonParser *p) {
    while (p->cur < p->end) {
        ...
        if (p->cur[1] == '*') {
            ...
            if (p->cur + 1 >= p->end) {
                json_parser_fail(p, RIVEN_JSON_ERROR_SYNTAX, "unterminated block comment");
                return;
            }
            p->cur += 2;
            continue;
...
static void out_init(JsonOut *out, int pretty, int indent) {
    ...
    if (!out->buf) riven_panic("out of memory");
    out->buf[0] = '\0';
}
...
static void out_reserve(JsonOut *out, size_t extra) {
    ...
    if (extra > SIZE_MAX - out->len - 1) {
        out->error_tag = RIVEN_JSON_ERROR_NUMBER_OUT_OF_RANGE;
        return;
    }
    ...
    if (!buf) riven_panic("out of memory");
    out->buf = buf;
    out->cap = next;
}
...
static char *parse_string(JsonParser *p) {
    if (!consume(p, '"')) {
        json_parser_fail(p, RIVEN_JSON_ERROR_SYNTAX, "expected string");
        return NULL;
    }
    ...
    while (p->cur < p->end) {
        ...
        if (c < 0x20) {
            out_free(&out);
            json_parser_fail(p, RIVEN_JSON_ERROR_SYNTAX, "control character in string");
            return NULL;
        }
        if (c != '\\') {
            out_byte(&out, (char)c);
        ...
        if (p->cur >= p->end) {
            out_free(&out);
            json_parser_fail(p, RIVEN_JSON_ERROR_SYNTAX, "unterminated escape");
            return NULL;
        }
        char esc = *p->cur++;
        switch (esc) {
            ...
            case 'u': {
                ...
                if (!parse_hex4(p, &cp)) {
                    out_free(&out);
                    json_parser_fail(p, RIVEN_JSON_ERROR_SYNTAX, "invalid unicode escape");
                    return NULL;
                }
                if (cp >= 0xD800 && cp <= 0xDBFF) {
                    if ((size_t)(p->end - p->cur) < 6 || p->cur[0] != '\\' || p->cur[1] != 'u') {
                        out_free(&out);
                        json_parser_fail(p, RIVEN_JSON_ERROR_INVALID_UTF8, "missing low surrogate");
                        return NULL;
                    }
                    p->cur += 2;
                    uint32_t low;
                    if (!parse_hex4(p, &low) || low < 0xDC00 || low > 0xDFFF) {
                        out_free(&out);
                        json_parser_fail(p, RIVEN_JSON_ERROR_INVALID_UTF8, "invalid low surrogate");
                        return NULL;
                    }
                ...
                if (!append_utf8(&out, cp)) {
                    out_free(&out);
                    json_parser_fail(p, RIVEN_JSON_ERROR_INVALID_UTF8, "invalid unicode codepoint");
                    return NULL;
                }
                break;
            }
            default:
                out_free(&out);
                json_parser_fail(p, RIVEN_JSON_ERROR_SYNTAX, "invalid escape");
                return NULL;
        }
    }

    out_free(&out);
    json_parser_fail(p, RIVEN_JSON_ERROR_SYNTAX, "unterminated string");
    return NULL;
}
...
static void *parse_array(JsonParser *p, int depth) {
    if (!consume(p, '[')) {
        json_parser_fail(p, RIVEN_JSON_ERROR_SYNTAX, "expected array");
        return NULL;
    }
    RivenVec *items = riven_vec_new();
    skip_ws(p);
    if (p->error_message) return NULL;
    if (consume(p, ']')) return json_value(RIVEN_JSON_ARRAY, (int64_t)items);
    ...
    for (;;) {
        ...
        if (p->error_message) return NULL;
        riven_vec_push(items, (int64_t)item);
        skip_ws(p);
        if (p->error_message) return NULL;
        if (consume(p, ']')) break;
        if (!consume(p, ',')) {
            json_parser_fail(p, RIVEN_JSON_ERROR_SYNTAX, "expected comma or closing bracket");
            return NULL;
        }
        skip_ws(p);
        if (p->error_message) return NULL;
        if (p->cur < p->end && *p->cur == ']') {
            ...
            json_parser_fail(p, RIVEN_JSON_ERROR_SYNTAX, "trailing comma in array");
            return NULL;
        }
    }

    return json_value(RIVEN_JSON_ARRAY, (int64_t)items);
}

static void *parse_object(JsonParser *p, int depth) {
    if (!consume(p, '{')) {
        json_parser_fail(p, RIVEN_JSON_ERROR_SYNTAX, "expected object");
        return NULL;
    }
    RivenHash *obj = riven_hash_new();
    obj->string_keys = 1;
    skip_ws(p);
    if (p->error_message) return NULL;
    if (consume(p, '}')) return json_value(RIVEN_JSON_OBJECT, (int64_t)obj);

    for (;;) {
        skip_ws(p);
        if (p->error_message) return NULL;
        if (p->cur >= p->end || *p->cur != '"') {
            json_parser_fail(p, RIVEN_JSON_ERROR_SYNTAX, "expected object key");
            return NULL;
        }
        ...
        skip_ws(p);
        if (!consume(p, ':')) {
            json_parser_fail(p, RIVEN_JSON_ERROR_SYNTAX, "expected colon after object key");
            return NULL;
        }
        skip_ws(p);
        void *value = parse_value(p, depth + 1);
        if (p->error_message) return NULL;
        riven_hash_insert(obj, (int64_t)key, (int64_t)value);
        skip_ws(p);
        if (p->error_message) return NULL;
        if (consume(p, '}')) break;
        if (!consume(p, ',')) {
            json_parser_fail(p, RIVEN_JSON_ERROR_SYNTAX, "expected comma or closing brace");
            return NULL;
        }
        skip_ws(p);
        if (p->error_message) return NULL;
        if (p->cur < p->end && *p->cur == '}') {
            ...
            json_parser_fail(p, RIVEN_JSON_ERROR_SYNTAX, "trailing comma in object");
            return NULL;
        }
    }
...
static void *parse_number(JsonParser *p) {
    ...
    if (consume(p, '-')) {
        if (p->cur >= p->end) {
            json_parser_fail(p, RIVEN_JSON_ERROR_SYNTAX, "invalid number");
            return NULL;
        }
    }
    ...
    if (consume(p, '0')) {
        if (p->cur < p->end && isdigit((unsigned char)*p->cur)) {
            json_parser_fail(p, RIVEN_JSON_ERROR_SYNTAX, "leading zero in number");
            return NULL;
        }
    ...
    if (p->cur < p->end && *p->cur == '.') {
        ...
        p->cur++;
        if (p->cur >= p->end || !isdigit((unsigned char)*p->cur)) {
            json_parser_fail(p, RIVEN_JSON_ERROR_SYNTAX, "invalid fraction");
            return NULL;
        }
        while (p->cur < p->end && isdigit((unsigned char)*p->cur)) p->cur++;
    }
    ...
    if (p->cur < p->end && (*p->cur == 'e' || *p->cur == 'E')) {
        ...
        if (p->cur >= p->end || !isdigit((unsigned char)*p->cur)) {
            json_parser_fail(p, RIVEN_JSON_ERROR_SYNTAX, "invalid exponent");
            return NULL;
        }
        while (p->cur < p->end && isdigit((unsigned char)*p->cur)) p->cur++;
    }
    ...
    if (!tmp) riven_panic("out of memory");
    ...
    if (is_float) {
        ...
        if (errno == ERANGE || !endptr || *endptr != '\0' || !isfinite(d)) {
            free(tmp);
            json_parser_fail(p, RIVEN_JSON_ERROR_NUMBER_OUT_OF_RANGE, "number out of range");
            return NULL;
        }
        free(tmp);
        return json_float(d);
    ...
    if (errno == ERANGE || !endptr || *endptr != '\0') {
        free(tmp);
        json_parser_fail(p, RIVEN_JSON_ERROR_NUMBER_OUT_OF_RANGE, "integer out of range");
        return NULL;
    }
    free(tmp);
...
static void *parse_value(JsonParser *p, int depth) {
    if (depth > RIVEN_JSON_MAX_DEPTH) {
        json_parser_fail(p, RIVEN_JSON_ERROR_DEPTH_LIMIT, "JSON nesting depth exceeded");
        return NULL;
    }
    skip_ws(p);
    if (p->error_message) return NULL;
    if (p->cur >= p->end) {
        json_parser_fail(p, RIVEN_JSON_ERROR_SYNTAX, "unexpected end of input");
        return NULL;
    }
    char c = *p->cur;
    ...
    if (c == '"') {
        ...
        if (p->error_message) return NULL;
        return json_value(RIVEN_JSON_STRING, (int64_t)s);
    }
    if (c == '[') return parse_array(p, depth);
    if (c == '{') return parse_object(p, depth);
    if (c == '-' || (c >= '0' && c <= '9')) return parse_number(p);

    json_parser_fail(p, RIVEN_JSON_ERROR_SYNTAX, "unexpected token");
    return NULL;
}
...
static void *json_parse_with_options(const char *input, int allow_comments, int allow_trailing_commas) {
    if (!input) {
        return riven_result_err_value((int64_t)json_error(
            RIVEN_JSON_ERROR_SYNTAX, "input is null"));
    }
    ...
    p.error_tag = RIVEN_JSON_ERROR_SYNTAX;
    ...
    if (p.error_message) return json_result_error(&p);
    if (p.cur != p.end) {
        json_parser_fail(&p, RIVEN_JSON_ERROR_SYNTAX, "trailing characters after JSON value");
        return json_result_error(&p);
    }
    return riven_result_ok_value((int64_t)value);
}
...
static void stringify_array(JsonOut *out, RivenVec *v, int depth) {
    out_byte(out, '[');
    if (v && v->len > 0) {
...
static void stringify_object(JsonOut *out, RivenHash *h, int depth) {
    out_byte(out, '{');
    int first = 1;
    if (h) {
...
static void stringify_value(JsonOut *out, int64_t raw, int depth) {
    if (out->error_tag >= 0) return;
    if (depth > RIVEN_JSON_MAX_DEPTH) {
        out->error_tag = RIVEN_JSON_ERROR_DEPTH_LIMIT;
        return;
    }
    ...
    int tag = *(int32_t *)v;
    switch (tag) {
        case RIVEN_JSON_NULL:
            out_cstr(out, "null");
            break;
        case RIVEN_JSON_BOOL:
        ...
        case RIVEN_JSON_STRING:
            stringify_string(out, (const char *)v[1]);
            break;
        case RIVEN_JSON_ARRAY:
            stringify_array(out, (RivenVec *)v[1], depth);
            break;
        case RIVEN_JSON_OBJECT:
            stringify_object(out, (RivenHash *)v[1], depth);
            break;
        default:
            out->error_tag = RIVEN_JSON_ERROR_SYNTAX;
            break;
    }
}
...
static void *json_stringify_with_options(void *value, int pretty, int64_t indent) {
    ...
    if (indent > INT_MAX) {
        return riven_result_err_value((int64_t)json_error(
            RIVEN_JSON_ERROR_NUMBER_OUT_OF_RANGE, "indent is too large"));
    }
    ...
    if (out.error_tag >= 0) {
        ...
        out_free(&out);
        return riven_result_err_value((int64_t)json_error(tag, "invalid Json value"));
    }
    char *result = out.buf;
    return riven_result_ok_value((int64_t)result);
}
...
void *riven_json_stringify_pretty(void *value, int64_t indent) {
    if (indent < 0) indent = 0;
    return json_stringify_with_options(value, 1, indent);
}

static int json_tag(void *value) {
    if (!value) return RIVEN_JSON_NULL;
...
void *riven_json_kind(void *value) {
    int tag = json_tag(value);
    if (tag < RIVEN_JSON_NULL || tag > RIVEN_JSON_OBJECT) tag = RIVEN_JSON_NULL;
    return json_value(tag, 0);
}

int8_t riven_json_is_null(void *value) {
    return json_tag(value) == RIVEN_JSON_NULL;
}

int8_t riven_json_is_bool(void *value) {
    return json_tag(value) == RIVEN_JSON_BOOL;
}

int8_t riven_json_is_int(void *value) {
    return json_tag(value) == RIVEN_JSON_INT;
}

int8_t riven_json_is_float(void *value) {
    return json_tag(value) == RIVEN_JSON_FLOAT;
}

int8_t riven_json_is_string(void *value) {
    return json_tag(value) == RIVEN_JSON_STRING;
}

int8_t riven_json_is_array(void *value) {
    return json_tag(value) == RIVEN_JSON_ARRAY;
}

int8_t riven_json_is_object(void *value) {
    return json_tag(value) == RIVEN_JSON_OBJECT;
}

void *riven_json_as_bool(void *value) {
    if (!riven_json_is_bool(value)) return option_none();
    return option_some(json_payload(value) ? 1 : 0);
}

void *riven_json_as_int(void *value) {
    if (!riven_json_is_int(value)) return option_none();
    return option_some(json_payload(value));
}

void *riven_json_as_float(void *value) {
    if (!riven_json_is_float(value)) return option_none();
    return option_some(json_payload(value));
}

void *riven_json_as_string(void *value) {
    if (!riven_json_is_string(value)) return option_none();
    return option_some((int64_t)riven_string_from((const char *)json_payload(value)));
}

void *riven_json_array_len(void *value) {
    if (!riven_json_is_array(value)) return option_none();
    RivenVec *items = (RivenVec *)json_payload(value);
    return option_some(items ? (int64_t)items->len : 0);
}

void *riven_json_object_len(void *value) {
    if (!riven_json_is_object(value)) return option_none();
    RivenHash *obj = (RivenHash *)json_payload(value);
...
int8_t riven_json_object_has(void *value, const char *key) {
    if (!riven_json_is_object(value)) return 0;
    return json_object_find((RivenHash *)json_payload(value), key) ? 1 : 0;
}
