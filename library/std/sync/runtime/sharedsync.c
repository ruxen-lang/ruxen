 ...
 *
 * The handle exposed to Riven is a pointer to the *payload* (the
...
int64_t riven_sharedsync_new(int64_t initial) {
    RivenSharedSync *s = (RivenSharedSync *)malloc(sizeof(RivenSharedSync));
    if (!s) riven_panic("SharedSync.new: out of memory");
    atomic_store_explicit(&s->refcount, 1, memory_order_release);
    s->payload = initial;
    return (int64_t)s;
}

int64_t riven_sharedsync_clone(int64_t ptr) {
    RivenSharedSync *s = (RivenSharedSync *)ptr;
    if (!s) riven_panic("SharedSync.clone: null handle");
    atomic_fetch_add_explicit(&s->refcount, 1, memory_order_acq_rel);
    return (int64_t)s;
}

int64_t riven_sharedsync_strong_count(int64_t ptr) {
    RivenSharedSync *s = (RivenSharedSync *)ptr;
    if (!s) return 0;
    return (int64_t)atomic_load_explicit(&s->refcount, memory_order_acquire);
}

int64_t riven_sharedsync_get(int64_t ptr) {
    RivenSharedSync *s = (RivenSharedSync *)ptr;
    if (!s) riven_panic("SharedSync.deref: null handle");
    return s->payload;
}

/* SharedSync drop — decrement and free on zero.
 *
 * On the final drop, the payload is left untouched. The Riven
 ...
 * codegen-driven.
 */
void riven_sharedsync_drop(int64_t ptr) {
    RivenSharedSync *s = (RivenSharedSync *)ptr;
    if (!s) return;
