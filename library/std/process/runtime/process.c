 ...
 *
 * Wire layouts (all int64-aligned; allocated via ruxen_alloc):
 *
 *   RuxenCommand  (64 bytes)
 ...
 *
 *   RuxenExitStatus (8 bytes)
 *     +0   int64    code             — POSIX-shell convention:
 *                                      0..=255 = normal exit;
 *                                      128+signal = signal termination;
 *                                      127 = fork/exec/waitpid failure
 *
 *   RuxenOutput (24 bytes)
 *     +0   RuxenExitStatus*  status  — sub-allocation, freed by Output_free
 *     +8   char*             stdout  — heap String (ruxen_string_from)
 ...
 *     and direct extern elsewhere — same approach `ruxen_env_vars`
 ...
 *     existing `ruxen_process_run` and to keep the implementation
...
_Static_assert(sizeof(RuxenCommand) == 64,
    "RuxenCommand wire layout drifted from documented 64-byte form");
...
static int ruxen_command_grow(int64_t *count, int64_t *cap, char ***arr) {
    if (*count + 1 > *cap) {
...
RuxenCommand *ruxen_command_new(const char *program) {
    RuxenCommand *c = (RuxenCommand *)ruxen_alloc(sizeof(RuxenCommand));
    /* ruxen_alloc panics on OOM, so c is non-null here.
     * We use plain malloc+memcpy (via `ruxen_string_from`-style copy)
     * for the inner strings rather than `ruxen_string_from` itself —
     * inner strings are freed with `free(...)` in `ruxen_command_free_inner`,
     * which is the same allocator pairing. Keeping the Command's
     * internal strings out of the ruxen-managed String pool means
     * they don't need to participate in the Ruxen drop tracker. */
    const char *src = program ? program : "";
    size_t len = strlen(src);
    c->program = (char *)malloc(len + 1);
    if (!c->program) ruxen_panic("out of memory");
...
RuxenCommand *ruxen_command_arg(RuxenCommand *c, const char *arg) {
    if (!c) return c;
    if (ruxen_command_grow(&c->args_count, &c->args_cap, &c->args) != 0) {
        ruxen_panic("out of memory");
    }
    const char *src = arg ? arg : "";
    size_t len = strlen(src);
    char *copy = (char *)malloc(len + 1);
    if (!copy) ruxen_panic("out of memory");
...
RuxenCommand *ruxen_command_args(RuxenCommand *c, RuxenVec *args) {
    if (!c) return c;
    if (!args) return c;
    for (uint64_t i = 0; i < args->len; i++) {
        const char *src = (const char *)(uintptr_t)args->data[i];
        ruxen_command_arg(c, src);
    }
    return c;
}

RuxenCommand *ruxen_command_env(RuxenCommand *c, const char *key, const char *value) {
    if (!c) return c;
    ...
    if (!entry) ruxen_panic("out of memory");
    memcpy(entry, k, klen);
    entry[klen] = '=';
    memcpy(entry + klen + 1, v, vlen);
    entry[klen + 1 + vlen] = '\0';
    if (ruxen_command_grow(&c->envs_count, &c->envs_cap, &c->envs) != 0) {
        free(entry);
        ruxen_panic("out of memory");
    }
...
RuxenCommand *ruxen_command_current_dir(RuxenCommand *c, const char *path) {
    if (!c) return c;
    if (c->cwd) {
        free(c->cwd);
        c->cwd = NULL;
    ...
    if (path) {
        ...
        if (!c->cwd) ruxen_panic("out of memory");
...
static void ruxen_command_free_inner(RuxenCommand *c) {
    if (!c) return;
...
/* Explicit Command free for the drop pass. Mirrors `ruxen_metadata_free`
 ...
 * leaking via the generic `ruxen_dealloc`. The MIR drop wiring (see
 * `insert_drops` in mir/lower.rs) maps Class types to `ruxen_dealloc`
 * by default, so we add `Command` to the `user_drop_classes` set so
 * `Command_drop(self)` is emitted before the spine dealloc. */
void ruxen_command_drop(RuxenCommand *c) {
    if (!c) return;
    ruxen_command_free_inner(c);
    /* Caller's drop pass will emit ruxen_dealloc(c) next. */
}
...
static char **ruxen_command_build_envp(RuxenCommand *c) {
    if (!c) return NULL;
...
static char **ruxen_command_build_argv(RuxenCommand *c) {
    if (!c) return NULL;
 ...
 *   normal exit:    0..=255
 *   signal:         128 + signum (matches `ruxen_process_run`)
 *   anything else:  127 (lump under the fork/exec failure code)
 */
static int64_t ruxen_command_pack_status(int wstatus) {
    if (WIFEXITED(wstatus))   return (int64_t)WEXITSTATUS(wstatus);
    if (WIFSIGNALED(wstatus)) return (int64_t)(128 + WTERMSIG(wstatus));
    return 127;
}

static RuxenExitStatus *ruxen_exit_status_alloc(int64_t code) {
    RuxenExitStatus *st = (RuxenExitStatus *)ruxen_alloc(sizeof(RuxenExitStatus));
    st->code = code;
    return st;
}
 ...
 * drop pass emits `Command_drop(c) + ruxen_dealloc(c)` at scope exit
...
void *ruxen_command_status(RuxenCommand *c) {
    ...
    if (!c->program || !*c->program) {
        return ruxen_io_error_message("command program is empty");
    }
    ...
    char **argv = ruxen_command_build_argv(c);
    char **envp = ruxen_command_build_envp(c);
    if (!argv || !envp) {
        if (argv) free(argv);
        if (envp) free(envp);
        return ruxen_io_error_message("out of memory");
    }
    ...
    if (pid == 0) {
        ...
        if (c->cwd && chdir(c->cwd) != 0) {
            int saved = errno;
            fprintf(stderr,
                    "ruxen_command_status: chdir(\"%s\") failed: %s (errno=%d)\n",
        ...
        int saved = errno;
        fprintf(stderr,
                "ruxen_command_status: execve(\"%s\") failed: %s (errno=%d)\n",
    ...
    int64_t code = ruxen_command_pack_status(wstatus);
    RuxenExitStatus *st = ruxen_exit_status_alloc(code);

    return ruxen_result_ok_value((int64_t)st);
}
 ...
 * zero-length string. */
static char *ruxen_command_drain_fd(int fd, int *err_out) {
    size_t cap = 256;
    size_t len = 0;
...
void *ruxen_command_output(RuxenCommand *c) {
    ...
    /* Pre-flight existence check — see `ruxen_command_status` for the
    ...
    if (pid == 0) {
        ...
        if (c->cwd && chdir(c->cwd) != 0) {
            ...
            int saved = errno;
            fprintf(stderr,
                    "ruxen_command_output: chdir(\"%s\") failed: %s (errno=%d)\n",
                    c->cwd, strerror(saved), saved);
            _exit(127);
        }
        execve(c->program, argv, envp);
        int saved = errno;
        fprintf(stderr,
                "ruxen_command_output: execve(\"%s\") failed: %s (errno=%d)\n",
                c->program, strerror(saved), saved);
        _exit(127);
    }
    ...
    int err_n = 0;
    char *stdout_buf = ruxen_command_drain_fd(out_pipe[0], &err_n);
    close(out_pipe[0]);
    if (!stdout_buf) {
        close(err_pipe[0]);
        ...
        return ruxen_io_error_from_errno(err_n);
    }
    char *stderr_buf = ruxen_command_drain_fd(err_pipe[0], &err_n);
    close(err_pipe[0]);
    if (!stderr_buf) {
        free(stdout_buf);
        int junk = 0;
    ...
    /* Re-wrap the raw read buffers into ruxen-managed Strings so the
     * Ruxen-side drop pass on the Output's stdout / stderr fields can
     * call `ruxen_string_free` uniformly. `ruxen_string_from` allocates
     * a fresh buffer and copies — we then free the read buffer. */
    char *stdout_riv = ruxen_string_from(stdout_buf);
    char *stderr_riv = ruxen_string_from(stderr_buf);
    free(stdout_buf);
    free(stderr_buf);

    RuxenOutput *out = (RuxenOutput *)ruxen_alloc(sizeof(RuxenOutput));
    out->status = ruxen_exit_status_alloc(ruxen_command_pack_status(wstatus));
...
/* `ruxen_exit_status_code` / `ruxen_exit_status_success` ported to
 * pure Ruxen via `layout c` (see `library/std/process/src/lib.rx`).
...
void ruxen_exit_status_free(RuxenExitStatus *st) {
    if (st) ruxen_dealloc(st);
}
 ...
 */
char *ruxen_output_stdout(RuxenOutput *o) {
    if (!o || !o->stdout_buf) return ruxen_string_from("");
    return ruxen_string_from(o->stdout_buf);
}

char *ruxen_output_stderr(RuxenOutput *o) {
    if (!o || !o->stderr_buf) return ruxen_string_from("");
    return ruxen_string_from(o->stderr_buf);
}

/* `.status()` returns a FRESH ExitStatus copy so the caller can drop
 * the Output without dangling pointers. */
RuxenExitStatus *ruxen_output_status(RuxenOutput *o) {
    if (!o) return ruxen_exit_status_alloc(127);
    int64_t code = o->status ? o->status->code : 127;
    return ruxen_exit_status_alloc(code);
}

/* Drop helper for Output — frees the sub-status, the two String
 * buffers, then the spine. Mirrors `ruxen_command_drop`. */
void ruxen_output_drop(RuxenOutput *o) {
    if (!o) return;
    if (o->status) {
        ruxen_dealloc(o->status);
        o->status = NULL;
    }
    if (o->stdout_buf) {
        free(o->stdout_buf);
